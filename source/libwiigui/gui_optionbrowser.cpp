...
GuiOptionBrowser::GuiOptionBrowser(int w, int h, OptionList * l, const char *imagebg, int scrollon)
{
    ...
    {
        optionTxt[i] = new GuiText((char *) NULL, 20, ( GXColor )
        {   0, 0, 0, 0xff});
        ...
        optionVal[i] = new GuiText((char *) NULL, 20, ( GXColor )
        {   0, 0, 0, 0xff});
...
void GuiOptionBrowser::Update(GuiTrigger * t)
{
    ...
    for (int i = 0; i < PAGESIZE; i++)
    {
        if (focus)
        {
            if (i != selectedItem && optionBtn[i]->GetState() == STATE_SELECTED)
                optionBtn[i]->ResetState();
            else if (i == selectedItem && optionBtn[i]->GetState() == STATE_DEFAULT) optionBtn[selectedItem]->SetState(
    ...
    if (scrollbaron == 1)
    {
        ...
        if (t->Down() || arrowDownBtn->GetState() == STATE_CLICKED || ////////////////////////////////////////////down
        ...
        else if (t->Up() || arrowUpBtn->GetState() == STATE_CLICKED || ////////////////////////////////////////////up
