 ...
 */
int ntfs_mft_records_read(const ntfs_volume *vol, const MFT_REF mref, const s64 count, MFT_RECORD *b)
{
    s64 br;
    VCN m;
 ...
 */
int ntfs_mft_records_write(const ntfs_volume *vol, const MFT_REF mref, const s64 count, MFT_RECORD *b)
{
    s64 bw;
    VCN m;
    void *bmirr = NULL;
...
int ntfs_file_record_read(const ntfs_volume *vol, const MFT_REF mref, MFT_RECORD **mrec, ATTR_RECORD **attr)
{
    ...
    if (ntfs_mft_record_read(vol, mref, m)) goto err_out;

    if (ntfs_mft_record_check(vol, mref, m)) goto err_out;
    ...
    *mrec = m;
    if (attr) *attr = (ATTR_RECORD*) ((char*) m + le16_to_cpu(m->attrs_offset));
    return 0;
    err_out: if (m != *mrec) free(m);
    return -1;
}
...
int ntfs_mft_record_format(const ntfs_volume *vol, const MFT_REF mref)
{
    ...
    if (ntfs_mft_record_layout(vol, mref, m)) goto free_m;

    if (ntfs_mft_record_write(vol, mref, m)) goto free_m;
...
{
    if (ni && ni->mft_no == FILE_MFT) return 1;
    return 0;
}
...
static int ntfs_mft_bitmap_find_free_rec(ntfs_volume *vol, ntfs_inode *base_ni)
{
    ...
    if (pass_end > ll) pass_end = ll;
    pass = 1;
    if (!base_ni)
        data_pos = vol->mft_data_pos;
    else data_pos = base_ni->mft_no + 1;
    if (data_pos < RESERVED_MFT_RECORDS) data_pos = RESERVED_MFT_RECORDS;
    ...
    for (; pass <= 2; size = PAGE_SIZE)
    {
        ...
        if (ll)
        {
            ...
            ntfs_log_debug("Before inner for loop: size 0x%x, "
                    ...
                    byte ? *byte : -1, b);
            for (; bit < size && data_pos + bit < pass_end; bit &= ~7ull, bit += 8)
            {
                /*
                 ...
                 */
                if (ntfs_is_mft(base_ni) && bit > 400) goto out;
...
static int ntfs_mft_bitmap_extend_allocation_i(ntfs_volume *vol)
{
     ...
     */
    rl = ntfs_attr_find_vcn(mftbmp_na, (mftbmp_na->allocated_size - 1) >> vol->cluster_size_bits);
    ...
    if (!rl)
    {
        ...
        ntfs_log_error("Failed to merge runlists for mft "
                "bitmap.\n");
        if (ntfs_cluster_free_from_rl(vol, rl2)) ntfs_log_error("Failed to deallocate "
                "cluster.%s\n", es);
        free(rl2);
        errno = err;
        return STATUS_ERROR;
    ...
    if (!ctx) goto undo_alloc;

    if (ntfs_attr_lookup(mftbmp_na->type, mftbmp_na->name, mftbmp_na->name_len, 0, rl[1].vcn, NULL, 0, ctx))
    {
    ...
    if (ntfs_attr_record_resize(m, a, mp_size + le16_to_cpu(a->mapping_pairs_offset)))
    {
        ntfs_log_info("extending $MFT bitmap\n");
        ret = ntfs_mft_attr_extend(vol->mftbmp_na);
        if (ret == STATUS_OK) goto ok;
        if (ret == STATUS_ERROR)
        {
    ...
    if (ntfs_mapping_pairs_build(vol, (u8*) a + le16_to_cpu(a->mapping_pairs_offset), mp_size, rl2, ll, NULL))
    {
    ...
    if (a->lowest_vcn)
    {
        ...
        if (ntfs_attr_lookup(mftbmp_na->type, mftbmp_na->name, mftbmp_na->name_len, 0, 0, NULL, 0, ctx))
        {
    ...
    ok: mftbmp_na->allocated_size += vol->cluster_size;
    ...
    if (mp_rebuilt)
    {
        if (ntfs_mapping_pairs_build(vol, (u8*) a + le16_to_cpu(a->mapping_pairs_offset), old_alen
                - le16_to_cpu(a->mapping_pairs_offset), rl2, ll, NULL)) ntfs_log_error("Failed to restore mapping "
                "pairs array.%s\n", es);
        if (ntfs_attr_record_resize(m, a, old_alen)) ntfs_log_error("Failed to restore attribute "
                "record.%s\n", es);
        ntfs_inode_mark_dirty(ctx->ntfs_ino);
    }
    if (update_mp)
    {
        if (ntfs_attr_update_mapping_pairs(vol->mftbmp_na, 0)) ntfs_log_perror("%s: MP update failed", __FUNCTION__);
    }
...
static int ntfs_mft_data_extend_allocation(ntfs_volume *vol)
{
     ...
     */
    rl = ntfs_attr_find_vcn(mft_na, (mft_na->allocated_size - 1) >> vol->cluster_size_bits);
    ...
    if (!rl)
    {
        ...
        ntfs_log_error("Failed to merge runlists for mft data "
                "attribute.\n");
        if (ntfs_cluster_free_from_rl(vol, rl2)) ntfs_log_error("Failed to deallocate clusters "
    ...
    if (ntfs_attr_lookup(mft_na->type, mft_na->name, mft_na->name_len, 0, rl[1].vcn, NULL, 0, ctx))
    {
        ntfs_log_error("Failed to find last attribute extent of "
                "mft data attribute.\n");
        goto undo_alloc;
    ...
    if (a->lowest_vcn)
    {
        ...
        ntfs_inode_mark_dirty(ctx->ntfs_ino);
        ntfs_attr_reinit_search_ctx(ctx);
        if (ntfs_attr_lookup(mft_na->type, mft_na->name, mft_na->name_len, 0, 0, NULL, 0, ctx))
        {
            ntfs_log_error("Failed to find first attribute "
    ...
    ok: mft_na->allocated_size += nr << vol->cluster_size_bits;
    ...
    errno = err;
    undo_alloc: err = errno;
    if (ntfs_cluster_free(vol, mft_na, old_last_vcn, -1) < 0) ntfs_log_error("Failed to free clusters from mft data "
            "attribute.%s\n", es);
    if (ntfs_rl_truncate(&mft_na->rl, old_last_vcn)) ntfs_log_error("Failed to truncate mft data attribute "
            "runlist.%s\n", es);
    if (mp_rebuilt)
    {
        if (ntfs_mapping_pairs_build(vol, (u8*) a + le16_to_cpu(a->mapping_pairs_offset), old_alen
                - le16_to_cpu(a->mapping_pairs_offset), rl2, ll, NULL)) ntfs_log_error("Failed to restore mapping pairs "
                "array.%s\n", es);
        if (ntfs_attr_record_resize(m, a, old_alen)) ntfs_log_error("Failed to restore attribute "
                "record.%s\n", es);
        ntfs_inode_mark_dirty(ctx->ntfs_ino);
    ...
    if (update_mp)
    {
        if (ntfs_attr_update_mapping_pairs(vol->mft_na, 0)) ntfs_log_perror("%s: MP update failed", __FUNCTION__);
    }
    if (ctx) ntfs_attr_put_search_ctx(ctx);
    errno = err;
    goto out;
...
static int ntfs_mft_record_init(ntfs_volume *vol, s64 size)
{
    ...
    {
        if (ntfs_mft_data_extend_allocation(vol) == STATUS_ERROR) goto out;
    ...
    while (size > mft_na->initialized_size)
    {
        ...
        if (mft_na->initialized_size > mft_na->data_size) mft_na->data_size = mft_na->initialized_size;
    ...
    if (ntfs_inode_sync(mft_na->ni)) goto undo_data_init;
    ...
    undo_data_init: mft_na->initialized_size = old_data_initialized;
...
static ntfs_inode *ntfs_mft_rec_alloc(ntfs_volume *vol)
{
    ...
    if (bit >= 0) goto found_free_rec;

    if (errno != ENOSPC) goto out;
    ...
    found_free_rec: if (ntfs_bitmap_set_bit(mftbmp_na, bit))
    {
    ...
    if (ll > mft_na->initialized_size) if (ntfs_mft_rec_init(vol, ll) < 0) goto undo_mftbmp_alloc;
    /*
    ...
    if (seq_no) m->sequence_number = seq_no;
    seq_no = usn;
    if (seq_no && seq_no != const_cpu_to_le16(0xffff)) *(le16*) ((u8*) m + le16_to_cpu(m->usa_ofs)) = usn;
    ...
    if (!(base_ni->nr_extents & 3))
    {
        ...
        {
            memcpy(extent_nis, base_ni->extent_nis, i - 4 * sizeof(ntfs_inode *));
    ...
    ni->flags = 0;
    ni->creation_time = ni->last_data_change_time = ni->last_mft_change_time = ni->last_access_time
            = ntfs_current_time();
    /* Update the default mft allocation position if it was used. */
    if (!base_ni) vol->mft_data_pos = bit + 1;
    ...
    undo_mftbmp_alloc: err = errno;
    if (ntfs_bitmap_clear_bit(mftbmp_na, bit)) ntfs_log_error("Failed to clear bit in mft bitmap.%s\n", es);
    errno = err;
    err_out: if (!errno) errno = EIO;
    ni = NULL;
    goto out;
}
...
else    ntfs_log_enter("Entering (allocating a base mft record)\n");
...
int ntfs_mft_record_free(ntfs_volume *vol, ntfs_inode *ni)
{
    ...
    bitmap_rollback: if (ntfs_bitmap_set_bit(vol->mftbmp_na, mft_no))
    ntfs_log_debug("Eeek! Rollback failed in ntfs_mft_record_free().  "
            "Leaving inconsistent metadata!\n");
    sync_rollback: ni->mrec->flags |= MFT_RECORD_IN_USE;
