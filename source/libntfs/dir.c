 ...
 */
ntfschar NTFS_INDEX_I30[5] = { const_cpu_to_le16('$'), const_cpu_to_le16('I'), const_cpu_to_le16('3'),
        const_cpu_to_le16('0'), const_cpu_to_le16('\0') };
ntfschar NTFS_INDEX_SII[5] = { const_cpu_to_le16('$'), const_cpu_to_le16('S'), const_cpu_to_le16('I'),
        const_cpu_to_le16('I'), const_cpu_to_le16('\0') };
ntfschar NTFS_INDEX_SDH[5] = { const_cpu_to_le16('$'), const_cpu_to_le16('S'), const_cpu_to_le16('D'),
        const_cpu_to_le16('H'), const_cpu_to_le16('\0') };
ntfschar NTFS_INDEX_O[3] = { const_cpu_to_le16('$'), const_cpu_to_le16('O'), const_cpu_to_le16('\0') };
ntfschar NTFS_INDEX_Q[3] = { const_cpu_to_le16('$'), const_cpu_to_le16('Q'), const_cpu_to_le16('\0') };
ntfschar NTFS_INDEX_R[3] = { const_cpu_to_le16('$'), const_cpu_to_le16('R'), const_cpu_to_le16('\0') };
...
int ntfs_dir_inode_hash(const struct CACHED_GENERIC *cached)
{
    ...
    if (!name) name = (const unsigned char*) path;
    return (((name[0] << 1) + name[1] + strlen((const char*) name)) % (2 * CACHE_INODE_SIZE));
}
...
static int inode_cache_compare(const struct CACHED_GENERIC *cached, const struct CACHED_GENERIC *wanted)
{
...
static int inode_cache_inv_compare(const struct CACHED_GENERIC *cached, const struct CACHED_GENERIC *wanted)
{
    int len;
    BOOL different;
    ...
    if (w->pathname)
    {
        ...
        different = !cached->variable || ((w->inum != MREF(c->inum)) && (strncmp(c->pathname, w->pathname, len)
                || ((c->pathname[len] != '\0') && (c->pathname[len] != '/'))));
    }
    else different = !c->pathname || (w->inum != MREF(c->inum));
    return (different);
}
...
static int lookup_cache_compare(const struct CACHED_GENERIC *cached, const struct CACHED_GENERIC *wanted)
{
    ...
    return (!c->name || (c->parent != w->parent) || (c->namesize != w->namesize) || memcmp(c->name, w->name,
            c->namesize));
}
...
static int lookup_cache_inv_compare(const struct CACHED_GENERIC *cached, const struct CACHED_GENERIC *wanted)
{
    const struct CACHED_LOOKUP *c = (const struct CACHED_LOOKUP*) cached;
    const struct CACHED_LOOKUP *w = (const struct CACHED_LOOKUP*) wanted;
    return (!c->name || (c->parent != w->parent) || (MREF(c->inum) != MREF(w->inum)));
}
 ...
 * allow exact matches.
 */
u64 ntfs_inode_lookup_by_name(ntfs_inode *dir_ni, const ntfschar *uname, const int uname_len)
{
    VCN vcn;
    u64 mref = 0;
    s64 br;
    ...
    ir = (INDEX_ROOT*) ((u8*) ctx->attr + le16_to_cpu(ctx->attr->value_offset));
    ...
    {
        /* Bounds checks. */
        if ((u8*) ie < (u8*) ctx->mrec || (u8*) ie + sizeof(INDEX_ENTRY_HEADER) > index_end || (u8*) ie
                + le16_to_cpu(ie->key_length) > index_end)
        {
         ...
         */
        rc = ntfs_names_full_collate(uname, uname_len, (ntfschar*) &ie->key.file_name.file_name,
                ie->key.file_name.file_name_length, case_sensitivity, vol->upcase, vol->upcase_len);
        /*
    ...
    {
        /* Bounds check. */
        if ((u8*) ie < (u8*) ia || (u8*) ie + sizeof(INDEX_ENTRY_HEADER) > index_end || (u8*) ie
                + le16_to_cpu(ie->key_length) > index_end)
        {
    ...
    eo_put_err_out: ntfs_attr_put_search_ctx(ctx);
    errno = eo;
    return -1;
    close_err_out: eo = errno;
    free(ia);
    ntfs_attr_close(ia_na);
    goto eo_put_err_out;
}
...
u64 ntfs_inode_lookup_by_mbsname(ntfs_inode *dir_ni, const char *name)
{
    ...
    {
        cached_name = ntfs_uppercase_mbs(name, dir_ni->vol->upcase, dir_ni->vol->upcase_len);
    ...
    if (const_name)
    {
        ...
        if (dir_ni->vol->lookup_cache)
        {
            ...
            cached = (struct CACHED_LOOKUP*) ntfs_fetch_cache(dir_ni->vol->lookup_cache, GENERIC(&item),
                    lookup_cache_compare);
            if (cached)
            {
                inum = cached->inum;
                if (inum == (u64) -1) errno = ENOENT;
            }
            else
            {
                ...
                if (uname_len >= 0)
                {
                    ...
                    ntfs_enter_cache(dir_ni->vol->lookup_cache, GENERIC(&item), lookup_cache_compare);
                    free(uname);
                }
                else inum = (s64) -1;
            }
...
void ntfs_inode_update_mbsname(ntfs_inode *dir_ni, const char *name, u64 inum)
{
    ...
    if (dir_ni->vol->lookup_cache)
    {
        ...
        if (item.name)
        {
            ...
            item.parent = dir_ni->mft_no;
            item.inum = inum;
            cached = (struct CACHED_LOOKUP*) ntfs_enter_cache(dir_ni->vol->lookup_cache, GENERIC(&item),
                    lookup_cache_compare);
            if (cached) cached->inum = inum;
            if (cached_name) free(cached_name);
        }
    }
#endif
...
ntfs_inode *ntfs_pathname_to_inode(ntfs_volume *vol, ntfs_inode *parent, const char *pathname)
{
...
#if CACHE_INODE_SIZE
    fullname = p;
    if (p[0] && (p[strlen(p) - 1] == PATH_SEP)) ntfs_log_error("Unnormalized path %s\n",ascii);
#endif
    if (parent)
    {
        ni = parent;
    ...
    else
    {
        ...
        if (*fullname)
        {
            ...
            cached = (struct CACHED_INODE*) ntfs_fetch_cache(vol->xinode_cache, GENERIC(&item), inode_cache_compare);
        }
        else cached = (struct CACHED_INODE*) NULL;
        if (cached)
        {
            /*
    ...
    while (p && *p)
    {
        ...
        if (!cached)
        {
            ...
            {
                item.inum = inum;
                ntfs_enter_cache(vol->xinode_cache, GENERIC(&item), inode_cache_compare);
            }
        }
#else
        len = ntfs_mbstoucs(p, &unicode);
        ...
        if (ni != parent) if (ntfs_inode_close(ni))
        {
            err = errno;
            goto out;
        }
    ...
    result = ni;
    ni = NULL;
    close: if (ni && (ni != parent)) if (ntfs_inode_close(ni) && !err) err = errno;
    out: free(ascii);
    free(unicode);
    if (err) errno = err;
    return result;
 ...
 */
static const ntfschar dotdot[3] = { const_cpu_to_le16('.'), const_cpu_to_le16('.'), const_cpu_to_le16('\0') };
 ...
 * callback.
 */
static int ntfs_filldir(ntfs_inode *dir_ni, s64 *pos, u8 ivcn_bits, const INDEX_TYPE index_type, index_union iu,
        INDEX_ENTRY *ie, void *dirent, ntfs_filldir_t filldir)
{
    ...
    if (index_type == INDEX_TYPE_ALLOCATION)
        *pos = (u8*) ie - (u8*) iu.ia + (sle64_to_cpu(
                iu.ia->index_block_vcn) << ivcn_bits) + dir_ni->vol->mft_record_size;
    else /* if (index_type == INDEX_TYPE_ROOT) */
    *pos = (u8*) ie - (u8*) iu.ir;
    /* Skip root directory self reference entry. */
    if (MREF_LE(ie->indexed_file) == FILE_root) return 0;
    if (ie->key.file_name.file_attributes & FILE_ATTR_I30_INDEX_PRESENT)
        dt_type = NTFS_DT_DIR;
    else if (fn->file_attributes & FILE_ATTR_SYSTEM)
        dt_type = NTFS_DT_UNKNOWN;
    else dt_type = NTFS_DT_REG;

    /* return metadata files and hidden files if requested */
    mref = le64_to_cpu(ie->indexed_file);
    metadata = (MREF(mref) != FILE_root) && (MREF(mref) < FILE_first_user);
    if ((!metadata && (NVolShowHidFiles(dir_ni->vol) || !(fn->file_attributes & FILE_ATTR_HIDDEN)))
            || (NVolShowSysFiles(dir_ni->vol) && (NVolShowHidFiles(dir_ni->vol) || metadata)))
    {
        if (NVolCaseSensitive(dir_ni->vol))
        {
            res = filldir(dirent, fn->file_name, fn->file_name_length, fn->file_name_type, *pos, mref, dt_type);
        }
        else
        {
            ...
            if (loname)
            {
                ...
                ntfs_name_locase(loname, fn->file_name_length, dir_ni->vol->locase, dir_ni->vol->upcase_len);
                res = filldir(dirent, loname, fn->file_name_length, fn->file_name_type, *pos, mref, dt_type);
                free(loname);
            }
            else res = -1;
        }
...
static MFT_REF ntfs_mft_get_parent_ref(ntfs_inode *ni)
{
    ...
    fn = (FILE_NAME_ATTR*) ((u8*) ctx->attr + le16_to_cpu(ctx->attr->value_offset));
    if ((u8*) fn + le32_to_cpu(ctx->attr->value_length) > (u8*) ctx->attr + le32_to_cpu(ctx->attr->length))
    {
 ...
 */
int ntfs_readdir(ntfs_inode *dir_ni, s64 *pos, void *dirent, ntfs_filldir_t filldir)
{
    ...
    else i_size = ia_na->data_size;

    rc = 0;

    /* Are we at end of dir yet? */
    if (*pos >= i_size + vol->mft_record_size) goto done;

    /* Emulate . and .. for all directories. */
    if (!*pos)
    {
        rc = filldir(dirent, dotdot, 1, FILE_NAME_POSIX, *pos, MK_MREF(dir_ni->mft_no,
                le16_to_cpu(dir_ni->mrec->sequence_number)), NTFS_DT_DIR);
        if (rc) goto err_out;
        ++*pos;
    }
    if (*pos == 1)
    {
        ...
        rc = filldir(dirent, dotdot, 2, FILE_NAME_POSIX, *pos, parent_mref, NTFS_DT_DIR);
        if (rc) goto err_out;
        ++*pos;
    }
    ...
    for (;; ie = (INDEX_ENTRY*) ((u8*) ie + le16_to_cpu(ie->length)))
    {
        ...
        /* Bounds checks. */
        if ((u8*) ie < (u8*) ctx->mrec || (u8*) ie + sizeof(INDEX_ENTRY_HEADER) > index_end || (u8*) ie
                + le16_to_cpu(ie->key_length) > index_end) goto dir_err_out;
        /* The last entry cannot contain a name. */
        if (ie->ie_flags & INDEX_ENTRY_END) break;

        if (!le16_to_cpu(ie->length)) goto dir_err_out;

        /* Skip index root entry if continuing previous readdir. */
        if (ir_pos > (u8*) ie - (u8*) ir) continue;
        /*
         * Submit the directory entry to ntfs_filldir(), which will
         * invoke the filldir() callback as appropriate.
         */
        rc = ntfs_filldir(dir_ni, pos, index_vcn_size_bits, INDEX_TYPE_ROOT, ir, ie, dirent, filldir);
        if (rc)
        {
            ntfs_attr_put_search_ctx(ctx);
            ctx = NULL;
    ...
    *pos = vol->mft_record_size;
    ...
    {
        find_next_index_buffer: bmp_pos++;
        bmp_buf_pos++;
        /* If we have reached the end of the bitmap, we are done. */
        if (bmp_pos >> 3 >= bmp_na->data_size) goto EOD;
        ia_pos = bmp_pos << index_block_size_bits;
        if (bmp_buf_pos >> 3 < bmp_buf_size) continue;
        /* Read next chunk from the index bitmap. */
        bmp_buf_pos = 0;
        if ((bmp_pos >> 3) + bmp_buf_size > bmp_na->data_size) bmp_buf_size = bmp_na->data_size - (bmp_pos >> 3);
        br = ntfs_attr_pread(bmp_na, bmp_pos >> 3, bmp_buf_size, bmp);
        if (br != bmp_buf_size)
        {
            if (br != -1) errno = EIO;
    ...
    for (;; ie = (INDEX_ENTRY*) ((u8*) ie + le16_to_cpu(ie->length)))
    {
        ...
        if (ia_pos - ia_start > (u8*) ie - (u8*) ia) continue;
        /*
         * Submit the directory entry to ntfs_filldir(), which will
         * invoke the filldir() callback as appropriate.
         */
        rc = ntfs_filldir(dir_ni, pos, index_vcn_size_bits, INDEX_TYPE_ALLOCATION, ia, ie, dirent, filldir);
        if (rc) goto err_out;
    }
    ...
    *pos = i_size + vol->mft_record_size;
    ...
    free(bmp);
    if (bmp_na) ntfs_attr_close(bmp_na);
    if (ia_na) ntfs_attr_close(ia_na);
 ...
 */
static ntfs_inode *__ntfs_create(ntfs_inode *dir_ni, le32 securid, ntfschar *name, u8 name_len, mode_t type, dev_t dev,
        ntfschar *target, int target_len)
{
    ...
    else si_len = offsetof(STANDARD_INFORMATION, v1_end);
...
else    clear_nino_flag(ni, v3_Extensions);
...
ntfs_inode *ntfs_create(ntfs_inode *dir_ni, le32 securid, ntfschar *name, u8 name_len, mode_t type)
{
...
ntfs_inode *ntfs_create_device(ntfs_inode *dir_ni, le32 securid, ntfschar *name, u8 name_len, mode_t type, dev_t dev)
{
...
ntfs_inode *ntfs_create_symlink(ntfs_inode *dir_ni, le32 securid, ntfschar *name, u8 name_len, ntfschar *target,
        int target_len)
{
    ...
    return __ntfs_create(dir_ni, securid, name, name_len, S_IFLNK, 0, target, target_len);
}
...
static int ntfs_check_unlinkable_dir(ntfs_inode *ni, FILE_NAME_ATTR *fn)
{
    ...
    if (!ret || errno != ENOTEMPTY) return ret;
    /*
     ...
     */
    if ((link_count == 1) || (link_count == 2 && fn->file_name_type == FILE_NAME_DOS))
    {
        errno = ENOTEMPTY;
 ...
 * Return 0 on success or -1 on error with errno set to the error code.
 */
int ntfs_delete(ntfs_volume *vol, const char *pathname, ntfs_inode *ni, ntfs_inode *dir_ni, ntfschar *name, u8 name_len)
{
    ...
    search: while (!ntfs_attr_lookup(AT_FILE_NAME, AT_UNNAMED, 0, CASE_SENSITIVE, 0, NULL, 0, actx))
    {
        char *s;
        BOOL case_sensitive = IGNORE_CASE;

        errno = 0;
        fn = (FILE_NAME_ATTR*) ((u8*) actx->attr + le16_to_cpu(actx->attr->value_offset));
        ...
        if (fn->file_name_type == FILE_NAME_POSIX || case_sensitive_match) case_sensitive = CASE_SENSITIVE;

        if (ntfs_names_are_equal(fn->file_name, fn->file_name_length, name, name_len, case_sensitive, ni->vol->upcase,
                ni->vol->upcase_len))
        {
    ...
    if (ntfs_index_remove(dir_ni, ni, fn, le32_to_cpu(actx->attr->value_length))) goto err_out;

    if (ntfs_attr_record_rm(actx)) goto err_out;
    ...
    ntfs_invalidate_cache(vol->lookup_cache, GENERIC(&lkitem), lookup_cache_inv_compare, CACHE_NOHASH);
#endif
#if CACHE_INODE_SIZE
    inum = ni->mft_no;
    if (pathname)
    ...
    item.inum = inum;
    count = ntfs_invalidate_cache(vol->xinode_cache, GENERIC(&item), inode_cache_inv_compare, CACHE_NOHASH);
    if (pathname && !count) ntfs_log_error("Could not delete inode cache entry for %s\n",
            pathname);
#endif
    ...
    ni = NULL;
    ok: ntfs_inode_update_times(dir_ni, NTFS_UPDATE_MCTIME);
    out: if (actx) ntfs_attr_put_search_ctx(actx);
 ...
 * Return 0 on success or -1 on error with errno set to the error code.
 */
static int ntfs_link_i(ntfs_inode *ni, ntfs_inode *dir_ni, ntfschar *name, u8 name_len, FILE_NAME_TYPE_FLAGS nametype)
{
    FILE_NAME_ATTR *fn = NULL;
    int fn_len, err;
    ...
    if (ntfs_attr_add(ni, AT_FILE_NAME, AT_UNNAMED, 0, (u8*) fn, fn_len))
    {
        ...
        if (ntfs_index_remove(dir_ni, ni, fn, fn_len)) goto rollback_failed;
        goto err_out;
    }
    ...
    ntfs_log_trace("Done.\n");
    return 0;
    rollback_failed: ntfs_log_error("Rollback failed. Leaving inconsistent metadata.\n");
    err_out: free(fn);
    errno = err;
    return -1;
}
