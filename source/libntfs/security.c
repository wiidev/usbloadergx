...
static ntfschar sii_stream[] = { const_cpu_to_le16('$'), const_cpu_to_le16('S'), const_cpu_to_le16('I'),
        const_cpu_to_le16('I'), const_cpu_to_le16(0) };
static ntfschar sdh_stream[] = { const_cpu_to_le16('$'), const_cpu_to_le16('S'), const_cpu_to_le16('D'),
        const_cpu_to_le16('H'), const_cpu_to_le16(0) };
...
static const GUID __zero_guid = { const_cpu_to_le32(0), const_cpu_to_le16(0), const_cpu_to_le16(0), { 0, 0, 0, 0, 0, 0,
        0, 0 } };
...
char *ntfs_guid_to_mbs(const GUID *guid, char *guid_str)
{
    ...
    if (!_guid_str)
    {
        ...
        if (!_guid_str) return _guid_str;
    }
    res = snprintf(_guid_str, 37, "%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
            (unsigned int) le32_to_cpu(guid->data1), le16_to_cpu(guid->data2), le16_to_cpu(guid->data3),
            guid->data4[0], guid->data4[1], guid->data4[2], guid->data4[3], guid->data4[4], guid->data4[5],
            guid->data4[6], guid->data4[7]);
    if (res == 36) return _guid_str;
    if (!guid_str) free(_guid_str);
    errno = EINVAL;
    return NULL;
}
...
char *ntfs_sid_to_mbs(const SID *sid, char *sid_str, size_t sid_str_size)
{
    ...
    else i = snprintf(s, cnt, "0x%llx", (unsigned long long) u);
    if (i < 0 || i >= cnt) goto err_out;
    s += i;
    cnt -= i;
    ...
    for (j = 0; j < sid->sub_authority_count; j++)
    {
        ...
        i = snprintf(s, cnt, "-%u", (unsigned int) le32_to_cpu(leauth));
        if (i < 0 || i >= cnt) goto err_out;
        s += i;
        cnt -= i;
    }
...
void ntfs_generate_guid(GUID *guid)
{
    ...
    {
        p[i] = (u8) (random() & 0xFF);
        if (i == 7) p[7] = (p[7] & 0x0F) | 0x40;
        if (i == 8) p[8] = (p[8] & 0x3F) | 0x80;
    }
}
...
static int ntfs_local_read(ntfs_inode *ni, ntfschar *stream_name, int stream_name_len, char *buf, size_t size,
        off_t offset)
{
    ntfs_attr *na = NULL;
    int res, total = 0;
    ...
    {
        if (offset + size > (size_t) na->data_size) size = na->data_size - offset;
        while (size)
        {
            res = ntfs_attr_pread(na, offset, size, buf);
            if ((off_t) res < (off_t) size) ntfs_log_perror("ntfs_attr_pread partial read "
                    "(%lld : %lld <> %d)",
                    (long long)offset,
                    (long long)size, res);
            if (res <= 0)
...
static int ntfs_local_write(ntfs_inode *ni, ntfschar *stream_name, int stream_name_len, char *buf, size_t size,
        off_t offset)
{
    ntfs_attr *na = NULL;
    int res, total = 0;
    ...
    while (size)
    {
        ...
        if (res < (s64) size) ntfs_log_perror("ntfs_attr_pwrite partial write (%lld: "
...
static int entersecurity_stuff(ntfs_volume *vol, off_t offs)
{
    ...
    if (stuff)
    {
        ...
        do
        {
            written = ntfs_local_write(vol->secure_ni, STREAM_SDS, 4, stuff, STUFFSZ, offs);
            if (written == STUFFSZ)
            {
                total += STUFFSZ;
                offs += STUFFSZ;
...
static int entersecurity_data(ntfs_volume *vol, const SECURITY_DESCRIPTOR_RELATIVE *attr, s64 attrsz, le32 hash,
        le32 keyid, off_t offs, int gap)
{
    int res;
    int written1;
    int written2;
    ...
    if (fullattr)
    {
         ...
         * the first copy
         */
        if (gap) memset(fullattr, 0, gap);
        memcpy(&fullattr[gap + sizeof(SECURITY_DESCRIPTOR_HEADER)], attr, attrsz);
        ...
        written1 = ntfs_local_write(vol->secure_ni, STREAM_SDS, 4, fullattr, fullsz, offs - gap);
        written2 = ntfs_local_write(vol->secure_ni, STREAM_SDS, 4, fullattr, fullsz, offs - gap + ALIGN_SDS_BLOCK);
        if ((written1 == fullsz) && (written2 == written1))
            res = 0;
        else errno = ENOSPC;
        free(fullattr);
    }
...
static int entersecurity_indexes(ntfs_volume *vol, s64 attrsz, le32 hash, le32 keyid, off_t offs)
{
    union
    {
        struct
    ...
    if (!ntfs_ie_add(xsii, (INDEX_ENTRY*) &newsii))
    {
        ...
        if (!ntfs_ie_add(xsdh, (INDEX_ENTRY*) &newsdh)) res = 0;
    }
    return (res);
}
...
static le32 entersecurityattr(ntfs_volume *vol, const SECURITY_DESCRIPTOR_RELATIVE *attr, s64 attrsz, le32 hash)
{
    union
    {
        struct
    ...
    olderrno = errno;
    found = !ntfs_index_lookup((char*) &keyid, sizeof(SII_INDEX_KEY), xsii);
    ...
    if (psii)
    {
         ...
         */
        if (xsii->is_in_root)
            entry = ntfs_ie_get_first((INDEX_HEADER*) &xsii->ir->index);
        else entry = ntfs_ie_get_first((INDEX_HEADER*) &xsii->ib->index);
        /*
    ...
    if (securid)
    {
        ...
        offs += gap + size;
        if ((offs + attrsz + sizeof(SECURITY_DESCRIPTOR_HEADER) - 1) & ALIGN_SDS_BLOCK)
        {
            offs = ((offs + attrsz + sizeof(SECURITY_DESCRIPTOR_HEADER) - 1) | (ALIGN_SDS_BLOCK - 1)) + 1;
        }
        if (!(offs & (ALIGN_SDS_BLOCK - 1))) entersecurity_stuff(vol, offs);
        /*
         ...
         */
        if (entersecurity_data(vol, attr, attrsz, hash, securid, offs, gap) || entersecurity_indexes(vol, attrsz, hash,
                securid, offs)) securid = const_cpu_to_le32(0);
    }
    ...
    if (ntfs_inode_sync(vol->secure_ni)) ntfs_log_perror("Could not sync $Secure\n");
    return (securid);
}
...
static le32 setsecurityattr(ntfs_volume *vol, const SECURITY_DESCRIPTOR_RELATIVE *attr, s64 attrsz)
{
    ...
    if (vol->secure_ni && xsdh && !vol->secure_reentry++)
    {
        ...
        olderrno = errno;
        found = !ntfs_index_lookup((char*) &key, sizeof(SDH_INDEX_KEY), xsdh);
        if (!found && (errno != ENOENT))
        ...
        else
        {
             ...
             * if so get next
             */
            if (entry->ie_flags & INDEX_ENTRY_END) entry = ntfs_index_next(entry, xsdh);
            do
            {
                collision = FALSE;
                psdh = (struct SDH*) entry;
                if (psdh)
                    size = (size_t) le32_to_cpu(psdh->datasize) - sizeof(SECURITY_DESCRIPTOR_HEADER);
                else size = 0;
                /* if hash is not the same, the key is not present */
                if (psdh && (size > 0) && (psdh->keyhash == hash))
                {
                    /* if hash is the same */
                    /* check the whole record */
                    realign.parts.dataoffsh = psdh->dataoffsh;
                    realign.parts.dataoffsl = psdh->dataoffsl;
                    offs = le64_to_cpu(realign.all) + sizeof(SECURITY_DESCRIPTOR_HEADER);
                    oldattr = (char*) ntfs_malloc(size);
                    if (oldattr)
                    {
                        rdsize = ntfs_local_read(vol->secure_ni, STREAM_SDS, 4, oldattr, size, offs);
                        found = (rdsize == size) && !memcmp(oldattr, attr, size);
                        free(oldattr);
            ...
            else
            {
                ...
                else
                {
                     ...
                     */
                    securid = entersecurityattr(vol, attr, attrsz, hash);
                }
            }
        }
    }
    if (--vol->secure_reentry) ntfs_log_perror("Reentry error, check no multithreading\n");
    return (securid);
}
...
static int update_secur_descr(ntfs_volume *vol, char *newattr, ntfs_inode *ni)
{
    int newattrsz;
    int written;
    int res;
    ...
    if ((vol->major_ver < 3) || !vol->secure_ni)
    {
        ...
        if (na)
        {
            ...
            if (!res)
            {
                written = (int) ntfs_attr_pwrite(na, (s64) 0, (s64) newattrsz, newattr);
            ...
            na = ntfs_attr_open(ni, AT_STANDARD_INFORMATION, AT_UNNAMED, 0);
            if (na)
            {
        ...
        else
        {
             ...
             * were none
             */
            res = ntfs_attr_add(ni, AT_SECURITY_DESCRIPTOR, AT_UNNAMED, 0, (u8*) newattr, (s64) newattrsz);
        }
#if !FORCE_FORMAT_v1x
    }
    else
    {
        ...
        securid = setsecurityattr(vol, (const SECURITY_DESCRIPTOR_RELATIVE*) newattr, (s64) newattrsz);
        if (securid)
        {
            na = ntfs_attr_open(ni, AT_STANDARD_INFORMATION, AT_UNNAMED, 0);
            if (na)
            {
                ...
                if (!test_nino_flag(ni, v3_Extensions))
                {
                    ...
                    res = ntfs_attr_truncate(na, (s64) sizeof(STANDARD_INFORMATION));
...
static int upgrade_secur_desc(ntfs_volume *vol, const char *attr, ntfs_inode *ni)
{
    int attrsz;
    int res;
    le32 securid;
    ...
    if ((vol->major_ver >= 3) && (ni->mft_no >= FILE_first_user))
    {
        attrsz = ntfs_attr_size(attr);
        securid = setsecurityattr(vol, (const SECURITY_DESCRIPTOR_RELATIVE*) attr, (s64) attrsz);
        if (securid)
        {
            na = ntfs_attr_open(ni, AT_STANDARD_INFORMATION, AT_UNNAMED, 0);
            if (na)
...
static BOOL groupmember(struct SECURITY_CONTEXT *scx, uid_t uid, gid_t gid)
{
    ...
    else
    {
        ...
        if (fd >= 0)
        {
            ...
            close(fd);
            if (!c) ntfs_log_error("No group record found in %s\n",filename);
        }
        else
...
static struct PERMISSIONS_CACHE *create_caches(struct SECURITY_CONTEXT *scx, u32 securindex)
{
    ...
    cache = (struct PERMISSIONS_CACHE*) ntfs_malloc(sizeof(struct PERMISSIONS_CACHE) + index1
    ...
    if (cache)
    {
        ...
        *scx->pseccache = cache;
        for (i = 0; i <= index1; i++)
            cache->cachetable[i] = (struct CACHED_PERMISSIONS*) NULL;
    }
    return (cache);
}
...
static int compare(const struct CACHED_SECURID *cached, const struct CACHED_SECURID *item)
{
#if POSIXACLS
    size_t csize;
    size_t isize;
...
static int leg_compare(const struct CACHED_PERMISSIONS_LEGACY *cached, const struct CACHED_PERMISSIONS_LEGACY *item)
{
...
static void resize_cache(struct SECURITY_CONTEXT *scx, u32 securindex)
{
    ...
    index1 = securindex >> CACHE_PERMISSIONS_BITS;
    newcnt = index1 + 1;
    if (newcnt <= ((CACHE_PERMISSIONS_SIZE + (1 << CACHE_PERMISSIONS_BITS) - 1) >> CACHE_PERMISSIONS_BITS))
    {
        ...
        newcache = (struct PERMISSIONS_CACHE*) ntfs_malloc(sizeof(struct PERMISSIONS_CACHE) + (newcnt - 1)
                * sizeof(struct CACHED_PERMISSIONS*));
        if (newcache)
        {
            memcpy(newcache, oldcache, sizeof(struct PERMISSIONS_CACHE) + (oldcnt - 1)
                    * sizeof(struct CACHED_PERMISSIONS*));
            free(oldcache);
            /* mark new entries as not valid */
            for (i = newcache->head.last + 1; i <= index1; i++)
                newcache->cachetable[i] = (struct CACHED_PERMISSIONS*) NULL;
...
#else
static struct CACHED_PERMISSIONS *enter_cache(struct SECURITY_CONTEXT *scx, ntfs_inode *ni, uid_t uid, gid_t gid,
        mode_t mode)
#endif
{
    struct CACHED_PERMISSIONS *cacheentry;
    ...
    if (test_nino_flag(ni, v3_Extensions) && ni->security_id)
    {
        ...
        if (pcache && (pcache->head.last >= index1) && pcache->cachetable[index1])
        {
        ...
        else
        {
            ...
            {
                cacheblock = (struct CACHED_PERMISSIONS*) malloc(sizeof(struct CACHED_PERMISSIONS)
            ...
            else cacheentry = (struct CACHED_PERMISSIONS*) NULL;
        }
    }
    else
    {
        ...
        if (ni->mrec->flags & MFT_RECORD_IS_DIRECTORY)
        {
            ...
            wanted.varsize = 0;
#endif
            legacy = (struct CACHED_PERMISSIONS_LEGACY*) ntfs_enter_cache(scx->vol->legacy_cache, GENERIC(&wanted),
...
static struct CACHED_PERMISSIONS *fetch_cache(struct SECURITY_CONTEXT *scx, ntfs_inode *ni)
{
    struct CACHED_PERMISSIONS *cacheentry;
    struct PERMISSIONS_CACHE *pcache;
    u32 securindex;
    ...
    if (test_nino_flag(ni, v3_Extensions) && (ni->security_id))
    {
        ...
        if (pcache && (pcache->head.last >= index1) && pcache->cachetable[index1])
        {
            ...
            if (!cacheentry->valid)
                cacheentry = (struct CACHED_PERMISSIONS*) NULL;
            else pcache->head.p_hits++;
            if (pcache) pcache->head.p_reads++;
        }
    }
#if CACHE_LEGACY_SIZE
    else
    {
        ...
        if (ni->mrec->flags & MFT_RECORD_IS_DIRECTORY)
        {
            ...
            wanted.variable = (void*) NULL;
            wanted.varsize = 0;
            legacy = (struct CACHED_PERMISSIONS_LEGACY*) ntfs_fetch_cache(scx->vol->legacy_cache, GENERIC(&wanted),
                    (cache_compare) leg_compare);
...
static char *retrievesecurityattr(ntfs_volume *vol, SII_INDEX_KEY id)
{
    ...
    {
        ntfs_index_ctx_reinit(xsii);
        found = !ntfs_index_lookup((char*) &id, sizeof(SII_INDEX_KEY), xsii);
        if (found)
        {
            psii = (struct SII*) xsii->entry;
            size = (size_t) le32_to_cpu(psii->datasize) - sizeof(SECURITY_DESCRIPTOR_HEADER);
            ...
            if (securattr)
            {
                rdsize = ntfs_local_read(ni, STREAM_SDS, 4, securattr, size, offs);
                if ((rdsize != size) || !ntfs_valid_descr(securattr, rdsize))
                {
        ...
        else if (errno != ENOENT) ntfs_log_perror("Inconsistency in index $SII");
    }
    if (!securattr)
    {
...
static char *getsecurityattr(ntfs_volume *vol, ntfs_inode *ni)
{
     ...
     * attribute
     */
    if (test_nino_flag(ni, v3_Extensions) && vol->secure_ni && ni->security_id)
    {
    ...
    else
    {
        ...
        readallsz = 0;
        securattr = ntfs_attr_readall(ni, AT_SECURITY_DESCRIPTOR, AT_UNNAMED, 0, &readallsz);
...
static int ntfs_get_perm(struct SECURITY_CONTEXT *scx, ntfs_inode *ni, mode_t request)
{
    const SECURITY_DESCRIPTOR_RELATIVE *phead;
    const struct CACHED_PERMISSIONS *cached;
    char *securattr;
    ...
    else
    {
        ...
        else
        {
            ...
            if (securattr)
            {
...
#if OWNERFROMACL
                usid = ntfs_acl_owner(securattr);
                perm = ntfs_build_permissions(securattr, usid, gsid, isdir);
                uid = ntfs_find_user(scx->mapping[MAPUSERS], usid);
#else
                usid = (const SID*)&
                securattr[le32_to_cpu(phead->owner)];
                 ...
                 * and upgrade option is selected
                 */
                if (!test_nino_flag(ni, v3_Extensions) && (perm >= 0) && (scx->vol->secure_flags & (1
                        << SECURITY_ADDSECURIDS)))
                {
                    upgrade_secur_desc(scx->vol, securattr, ni);
                    /*
                     * fetch owner and group for cacheing
                     * if there is a securid
                     */
                }
                if (test_nino_flag(ni, v3_Extensions) && (perm >= 0))
                {
                    enter_cache(scx, ni, uid, gid, perm);
                }
                free(securattr);
            }
            else
        ...
        if (perm >= 0)
        {
             ...
             */
            if ((gid == scx->gid) || ((((perm >> 3) ^ perm) & (request >> 6) & 7) && groupmember(scx, scx->uid, gid)))
                perm &= 07070;
            else perm &= 07007;
        }
    }
...
int ntfs_get_owner_mode(struct SECURITY_CONTEXT *scx, ntfs_inode * ni, struct stat *stbuf)
{
    const SECURITY_DESCRIPTOR_RELATIVE *phead;
    char *securattr;
    const SID *usid; /* owner of file/directory */
        ...
        else
        {
            ...
            if (securattr)
            {
                ...
                if (perm >= 0)
                {
                     ...
                     * and upgrade option is selected
                     */
                    if (!test_nino_flag(ni, v3_Extensions) && (scx->vol->secure_flags & (1 << SECURITY_ADDSECURIDS)))
                    {
                        upgrade_secur_desc(scx->vol, securattr, ni);
                    }
#if OWNERFROMACL
                    ...
                    free(pxdesc);
#else
                    enter_cache(scx, ni, stbuf->st_uid, stbuf->st_gid, perm);
#endif
                }
                free(securattr);
            }
...
le32 ntfs_alloc_securid(struct SECURITY_CONTEXT *scx, uid_t uid, gid_t gid, mode_t mode, BOOL isdir)
{
    ...
    wanted.variable = (void*) NULL;
    wanted.varsize = 0;
    cached = (const struct CACHED_SECURID*) ntfs_fetch_cache(scx->vol->securid_cache, GENERIC(&wanted),
            (cache_compare) compare);
    /* quite simple, if we are lucky */
    if (cached) securid = cached->securid;
    ...
    if (!cached && (scx->vol->major_ver >= 3))
    {
        ...
        {
            newattrsz = ntfs_attr_size(newattr);
            securid = setsecurityattr(scx->vol, (const SECURITY_DESCRIPTOR_RELATIVE*) newattr, newattrsz);
            if (securid)
            {
                /* update cache, for subsequent use */
                wanted.securid = securid;
                ntfs_enter_cache(scx->vol->securid_cache, GENERIC(&wanted), (cache_compare) compare);
            }
            free(newattr);
        }
        else
        ...
        if (newattr)
        {
            ...
            if (!res)
            {
                ...
                if (!isdir)
                {
                    if (mode & S_IWUSR)
                        ni->flags &= ~FILE_ATTR_READONLY;
                    else ni->flags |= FILE_ATTR_READONLY;
                ...
                if (isdir && !ni->security_id)
                {
                    ...
                    legacy.varsize = 0;
#endif
                    ntfs_invalidate_cache(scx->vol->legacy_cache, GENERIC(&legacy), (cache_compare) leg_compare, 0);
                }
#endif
            }
            free(newattr);
...
int ntfs_set_mode(struct SECURITY_CONTEXT *scx, ntfs_inode *ni, mode_t mode)
{
    ...
    if (!res)
    {
        ...
        if (!processuid || (uid == processuid))
        {
             ...
             * not match process group
             */
            if (processuid && (gid != scx->gid) && !groupmember(scx, scx->uid, gid)) mode &= ~S_ISGID;
#if POSIXACLS
            if (newpxdesc)
            {
                newpxdesc->mode = mode;
...
int ntfs_sd_add_everyone(ntfs_inode *ni)
{
     ...
     */
    sd_len = sizeof(SECURITY_DESCRIPTOR_ATTR) + 2 * (sizeof(SID) + 4) + sizeof(ACL) + sizeof(ACCESS_ALLOWED_ACE);
    ...
    if (ret) ntfs_log_perror("Failed to add initial SECURITY_DESCRIPTOR");
...
int ntfs_allowed_access(struct SECURITY_CONTEXT *scx, ntfs_inode *ni, int accesstype) /* access type required (S_Ixxx values) */
{
    int perm;
    int res;
    int allow;
     ...
     * Also always allow if no mapping has been defined
     */
    if (!scx->mapping[MAPUSERS] || (!scx->uid && (!(accesstype & S_IEXEC)
        ...
        if (perm >= 0)
        {
            ...
            switch (accesstype)
            {
                ...
                case S_IWRITE + S_IEXEC:
                    allow = ((perm & (S_IWUSR | S_IWGRP | S_IWOTH)) != 0) && ((perm & (S_IXUSR | S_IXGRP | S_IXOTH))
                            != 0);
                    break;
                case S_IREAD:
                ...
                case S_IREAD + S_IEXEC:
                    allow = ((perm & (S_IRUSR | S_IRGRP | S_IROTH)) != 0) && ((perm & (S_IXUSR | S_IXGRP | S_IXOTH))
                            != 0);
                    break;
                case S_IREAD + S_IWRITE:
                    allow = ((perm & (S_IRUSR | S_IRGRP | S_IROTH)) != 0) && ((perm & (S_IWUSR | S_IWGRP | S_IWOTH))
                            != 0);
                    break;
                case S_IWRITE + S_IEXEC + S_ISVTX:
                    if (perm & S_ISVTX)
                    {
                        if ((ntfs_get_owner_mode(scx, ni, &stbuf) >= 0) && (stbuf.st_uid == scx->uid))
                            allow = 1;
                        else allow = 2;
                    }
                    else allow = ((perm & (S_IWUSR | S_IWGRP | S_IWOTH)) != 0) && ((perm
                            & (S_IXUSR | S_IXGRP | S_IXOTH)) != 0);
                    break;
                case S_IREAD + S_IWRITE + S_IEXEC:
                    allow = ((perm & (S_IRUSR | S_IRGRP | S_IROTH)) != 0) && ((perm & (S_IWUSR | S_IWGRP | S_IWOTH))
                            != 0) && ((perm & (S_IXUSR | S_IXGRP | S_IXOTH)) != 0);
                    break;
                default:
                    res = EINVAL;
                    allow = 0;
...
int ntfs_set_owner(struct SECURITY_CONTEXT *scx, ntfs_inode *ni, uid_t uid, gid_t gid)
{
    ...
    else
    {
        ...
        if (oldattr)
        {
            ...
            res = -1;
#else
            mode = perm = ntfs_build_permissions(oldattr, usid, gsid, isdir);
            if (perm >= 0)
            {
                fileuid = ntfs_find_user(scx->mapping[MAPUSERS], usid);
                filegid = ntfs_find_group(scx->mapping[MAPGROUPS], gsid);
    ...
    if (!res)
    {
        ...
        if (!scx->uid || ((((int) uid < 0) || (uid == fileuid)) && ((gid == scx->gid)
                || groupmember(scx, scx->uid, gid)) && (fileuid == scx->uid)))
        {
            /* replace by the new usid and gsid */
            /* or reuse old gid and sid for cacheing */
            if ((int) uid < 0) uid = fileuid;
            if ((int) gid < 0) gid = filegid;
            /* clear setuid and setgid if owner has changed */
            /* unless request originated by root */
            if (uid && (fileuid != uid)) mode &= 01777;
#if POSIXACLS
            res = ntfs_set_owner_mode(scx, ni, uid, gid,
                    mode, pxdesc);
#else
...
int ntfs_set_ownmod(struct SECURITY_CONTEXT *scx, ntfs_inode *ni, uid_t uid, gid_t gid, const mode_t mode)
{
    const SECURITY_DESCRIPTOR_RELATIVE *phead;
    const struct CACHED_PERMISSIONS *cached;
    char *oldattr;
...
static le32 build_inherited_id(struct SECURITY_CONTEXT *scx, const char *parentattr, BOOL fordir)
{
    ...
    if (scx->mapping[MAPUSERS])
    {
        ...
        if (!usid) usid = adminsid;
        if (!gsid) gsid = adminsid;
    }
    else
    {
        /*
    ...
    if (newattr)
    {
        ...
        pos += usidsz;
        securid = setsecurityattr(scx->vol, (SECURITY_DESCRIPTOR_RELATIVE*) newattr, pos);
        free(newattr);
    }
    else securid = const_cpu_to_le32(0);
    return (securid);
}
...
le32 ntfs_inherited_id(struct SECURITY_CONTEXT *scx, ntfs_inode *dir_ni, BOOL fordir)
{
     ...
     */
    if (test_nino_flag(dir_ni, v3_Extensions) && dir_ni->security_id)
    {
        cached = fetch_cache(scx, dir_ni);
        if (cached) securid = (fordir ? cached->inh_dirid : cached->inh_fileid);
    }
    /*
    ...
    if (!securid)
    {
        ...
        if (parentattr)
        {
            securid = build_inherited_id(scx, parentattr, fordir);
            free(parentattr);
            /*
            ...
            if (securid)
            {
                ...
                if (cached)
                {
                    ...
                    else cached->inh_fileid = securid;
                }
            }
        }
    }
...
static int link_single_group(struct MAPPING *usermapping, struct passwd *user, gid_t gid)
{
    struct group *group;
    char **grmem;
    int grcnt;
    ...
    if (group && group->gr_mem)
    {
        ...
        if (*grmem)
        {
            ...
            else groups = (gid_t*) realloc(groups, (grcnt + 1) * sizeof(gid_t));
            if (groups)
                groups[grcnt++] = gid;
            else
            {
...
static int link_group_members(struct SECURITY_CONTEXT *scx)
{
    ...
    res = 0;
    for (usermapping = scx->mapping[MAPUSERS]; usermapping && !res; usermapping = usermapping->next)
    {
        usermapping->grcnt = 0;
        ...
        {
            for (groupmapping = scx->mapping[MAPGROUPS]; groupmapping && !res; groupmapping = groupmapping->next)
            {
                if (link_single_group(usermapping, user, groupmapping->xid)) res = -1;
            }
            if (!res && link_single_group(usermapping, user, (gid_t) 0)) res = -1;
        }
    }
    return (res);
}
...
static int ntfs_do_default_mapping(struct SECURITY_CONTEXT *scx, uid_t uid, gid_t gid, const SID *usid)
{
    struct MAPPING *usermapping;
    struct MAPPING *groupmapping;
    SID *sid;
...
{
    return (ntfs_local_read((ntfs_inode*) fileid, AT_UNNAMED, 0, buf, size, offs));
}
...
int ntfs_build_mapping(struct SECURITY_CONTEXT *scx, const char *usermap_path, BOOL allowdef)
{
    ...
    static struct
    {
        ...
        le32 level4;
        le32 level5;
    } defmap = { 1, 5, const_cpu_to_be16(0), const_cpu_to_be32(5), const_cpu_to_le32(21),
            const_cpu_to_le32(DEFSECAUTH1), const_cpu_to_le32(DEFSECAUTH2), const_cpu_to_le32(DEFSECAUTH3),
    ...
    if (usermap_path[0] == '/')
    {
        ...
        else firstitem = (struct MAPLIST*) NULL;
    }
    else
    {
    ...
    else
    {
        ...
        if (allowdef)
        {
            if (!ntfs_do_default_mapping(scx, 0, 0, (const SID*) &defmap)) ntfs_log_info("Using default user mapping\n");
        }
    }
...
static BOOL feedsecurityattr(const char *attr, u32 selection, char *buf, u32 buflen, u32 *psize)
{
    const SECURITY_DESCRIPTOR_RELATIVE *phead;
    SECURITY_DESCRIPTOR_RELATIVE *pnhead;
    const ACL *pdacl;
    ...
    {
        if (selection & OWNER_SECURITY_INFORMATION) control |= phead->control & SE_OWNER_DEFAULTED;
        if (selection & GROUP_SECURITY_INFORMATION) control |= phead->control & SE_GROUP_DEFAULTED;
        if (selection & DACL_SECURITY_INFORMATION) control |= phead->control & (SE_DACL_PRESENT | SE_DACL_DEFAULTED
                | SE_DACL_AUTO_INHERITED | SE_DACL_PROTECTED);
        if (selection & SACL_SECURITY_INFORMATION) control |= phead->control & (SE_SACL_PRESENT | SE_SACL_DEFAULTED
        ...
        else pnhead->dacl = const_cpu_to_le32(0);
        ...
        else pnhead->sacl = const_cpu_to_le32(0);
        ...
        else pnhead->owner = const_cpu_to_le32(0);
        ...
        else pnhead->group = const_cpu_to_le32(0);
        if (pos != size) ntfs_log_error("Error in security descriptor size\n");
        *psize = size;
        ok = TRUE;
    }
...
static BOOL mergesecurityattr(ntfs_volume *vol, const char *oldattr, const char *newattr, u32 selection, ntfs_inode *ni)
{
    ...
    if (target)
    {
         ...
         */
        if ((selection & DACL_SECURITY_INFORMATION) ? newhead->dacl : oldhead->dacl)
        {
        ...
        else targhead->dacl = const_cpu_to_le32(0);
        if (selection & DACL_SECURITY_INFORMATION)
        {
            control |= newhead->control & (SE_DACL_PRESENT | SE_DACL_DEFAULTED | SE_DACL_PROTECTED);
            if (newhead->control & SE_DACL_AUTO_INHERIT_REQ) control |= SE_DACL_AUTO_INHERITED;
        }
        else control |= oldhead->control & (SE_DACL_PRESENT | SE_DACL_DEFAULTED | SE_DACL_AUTO_INHERITED
                | SE_DACL_PROTECTED);
        /*
         * copy new SACL if selected
         * or keep old SACL if any
         */
        if ((selection & SACL_SECURITY_INFORMATION) ? newhead->sacl : oldhead->sacl)
        {
        ...
        else targhead->sacl = const_cpu_to_le32(0);
        if (selection & SACL_SECURITY_INFORMATION)
        {
            control |= newhead->control & (SE_SACL_PRESENT | SE_SACL_DEFAULTED | SE_SACL_PROTECTED);
            if (newhead->control & SE_SACL_AUTO_INHERIT_REQ) control |= SE_SACL_AUTO_INHERITED;
        }
        else control |= oldhead->control & (SE_SACL_PRESENT | SE_SACL_DEFAULTED | SE_SACL_AUTO_INHERITED
                | SE_SACL_PROTECTED);
        /*
         * copy new OWNER if selected
         * or keep old OWNER if any
         */
        if ((selection & OWNER_SECURITY_INFORMATION) ? newhead->owner : oldhead->owner)
        {
        ...
        else targhead->owner = const_cpu_to_le32(0);
        if (selection & OWNER_SECURITY_INFORMATION)
            control |= newhead->control & SE_OWNER_DEFAULTED;
        else control |= oldhead->control & SE_OWNER_DEFAULTED;
        /*
         * copy new GROUP if selected
         * or keep old GROUP if any
         */
        if ((selection & GROUP_SECURITY_INFORMATION) ? newhead->group : oldhead->group)
        {
        ...
        else targhead->group = const_cpu_to_le32(0);
        if (selection & GROUP_SECURITY_INFORMATION)
            control |= newhead->control & SE_GROUP_DEFAULTED;
        else control |= oldhead->control & SE_GROUP_DEFAULTED;
        ...
        u32 *psize)
{
    ...
    if (scapi && (scapi->magic == MAGIC_API))
    {
        ...
        if (ni)
        {
            ...
            if (attr)
            {
                if (feedsecurityattr(attr, selection, buf, buflen, psize))
                {
                    if (test_nino_flag(ni, v3_Extensions) && ni->security_id)
                        res = le32_to_cpu(
                                ni->security_id);
    ...
    else errno = EINVAL; /* do not clear *psize */
    return (res);
}
...
int ntfs_set_file_security(struct SECURITY_API *scapi, const char *path, u32 selection, const char *attr)
{
    ...
    if (scapi && (scapi->magic == MAGIC_API) && attr)
    {
        ...
        missing
                = ((selection & OWNER_SECURITY_INFORMATION) && !phead->owner && !(phead->control & SE_OWNER_DEFAULTED))
                        || ((selection & GROUP_SECURITY_INFORMATION) && !phead->group && !(phead->control
                                & SE_GROUP_DEFAULTED));
        if (!missing && (phead->control & SE_SELF_RELATIVE) && ntfs_valid_descr(attr, attrsz))
        {
            ni = ntfs_pathname_to_inode(scapi->security.vol, NULL, path);
            if (ni)
            {
                oldattr = getsecurityattr(scapi->security.vol, ni);
                if (oldattr)
                {
                    if (mergesecurityattr(scapi->security.vol, oldattr, attr, selection, ni))
                    {
...
int ntfs_get_file_attributes(struct SECURITY_API *scapi, const char *path)
{
    ...
    if (scapi && (scapi->magic == MAGIC_API) && path)
    {
        ...
        if (ni)
        {
            ...
            if (ni->mrec->flags & MFT_RECORD_IS_DIRECTORY)
                attrib |= const_le32_to_cpu(FILE_ATTR_DIRECTORY);
            else attrib &= ~const_le32_to_cpu(FILE_ATTR_DIRECTORY);
            if (!attrib) attrib |= const_le32_to_cpu(FILE_ATTR_NORMAL);
    ...
    if (scapi && (scapi->magic == MAGIC_API) && path)
    {
        ...
        if (ni)
        {
            ...
            if (ni->mrec->flags & MFT_RECORD_IS_DIRECTORY)
            {
                 ...
                 */
                settable |= FILE_ATTR_COMPRESSED;
                if ((ni->flags ^ cpu_to_le32(attrib)) & FILE_ATTR_COMPRESSED)
                {
                    if (ni->flags & FILE_ATTR_COMPRESSED)
                        dirflags = const_cpu_to_le16(0);
                    else dirflags = ATTR_IS_COMPRESSED;
                    res = ntfs_attr_set_flags(ni, AT_INDEX_ROOT, NTFS_INDEX_I30, 4, dirflags, ATTR_COMPRESSION_MASK);
                }
            }
            if (!res)
            {
                ni->flags = (ni->flags & ~settable) | (cpu_to_le32(attrib) & settable);
                NInoSetDirty(ni);
            }
            if (!ntfs_inode_close(ni)) res = -1;
        }
        else errno = ENOENT;
    }
    return (res);
...
int ntfs_read_sds(struct SECURITY_API *scapi, char *buf, u32 size, u32 offset)
{
    ...
    if (scapi && (scapi->magic == MAGIC_API))
    {
        if (scapi->security.vol->secure_ni)
            got = ntfs_local_read(scapi->security.vol->secure_ni, STREAM_SDS, 4, buf, size, offset);
...
INDEX_ENTRY *ntfs_read_sii(struct SECURITY_API *scapi, INDEX_ENTRY *entry)
{
    ...
    if (scapi && (scapi->magic == MAGIC_API))
    {
        ...
        if (xsii)
        {
            ...
            {
                key.security_id = const_cpu_to_le32(0);
                found = !ntfs_index_lookup((char*) &key, sizeof(SII_INDEX_KEY), xsii);
                /* not supposed to find */
                if (!found && (errno == ENOENT)) ret = xsii->entry;
            }
            else ret = ntfs_index_next(entry, xsii);
    ...
    if (scapi && (scapi->magic == MAGIC_API))
    {
        ...
        if (xsdh)
        {
            if (!entry)
            {
                ...
                found = !ntfs_index_lookup((char*) &key, sizeof(SDH_INDEX_KEY), xsdh);
                /* not supposed to find */
                if (!found && (errno == ENOENT)) ret = xsdh->entry;
            }
            else ret = ntfs_index_next(entry, xsdh);
            if (!ret) errno = ENODATA;
        }
        else errno = ENOTSUP;
    }
...
struct SECURITY_API *ntfs_initialize_file_security(const char *device, int flags)
{
    ...
    if (!mnt && !(mntflag & NTFS_MF_MOUNTED) && !getuid())
    {
        ...
        if (vol)
        {
            scapi = (struct SECURITY_API*) ntfs_malloc(sizeof(struct SECURITY_API));
            if (!ntfs_volume_get_free_space(vol) && scapi)
            {
