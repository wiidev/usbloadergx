...
{
    if (ni->nr_extents == -1) return ni->base_ni;
    return ni;
}
...
{
    if (NInoDirty(ni)) ntfs_log_error("Releasing dirty inode %lld!\n",
            (long long)ni->mft_no);
...
static ntfs_inode *ntfs_inode_real_open(ntfs_volume *vol, const MFT_REF mref)
{
    ...
    {
        if (!ni->mrec->base_mft_record) ntfs_log_perror("No STANDARD_INFORMATION in base record"
    ...
    olderrno = errno;
    if (ntfs_attr_lookup(AT_ATTRIBUTE_LIST, AT_UNNAMED, 0, CASE_SENSITIVE, 0, NULL, 0, ctx))
    {
    ...
    else
    {
        if (ctx->attr->non_resident)
        {
            ...
            else ni->allocated_size = sle64_to_cpu(
    ...
    err_out: __ntfs_inode_release(ni);
    ni = NULL;
    goto out;
}
...
int ntfs_inode_real_close(ntfs_inode *ni)
{
    ...
    else if (ni->nr_extents == -1)
    {
        ...
        for (i = 0; i < base_ni->nr_extents; ++i)
        {
            ...
            if (tmp_nis[i] != ni) continue;
            /* Found it. Disconnect. */
            memmove(tmp_nis + i, tmp_nis + i + 1, (base_ni->nr_extents - i - 1) * sizeof(ntfs_inode *));
            ...
            if (base_ni->nr_extents)
            {
                ...
                tmp_nis = realloc(tmp_nis, base_ni->nr_extents * sizeof(ntfs_inode *));
                /* Ignore errors, they don't really matter. */
                if (tmp_nis) base_ni->extent_nis = tmp_nis;
            }
            else if (tmp_nis)
            {
                free(tmp_nis);
         ...
         */
        if (i != -1) ntfs_log_error("Extent inode %lld was not found\n",
                (long long)ni->mft_no);
    }
...
{
    return (((const struct CACHED_NIDATA*) item)->inum % (2 * CACHE_NIDATA_SIZE));
}
...
static int idata_cache_compare(const struct CACHED_GENERIC *cached, const struct CACHED_GENERIC *wanted)
{
    return (((const struct CACHED_NIDATA*) cached)->inum != ((const struct CACHED_NIDATA*) wanted)->inum);
}
...
void ntfs_inode_invalidate(ntfs_volume *vol, const MFT_REF mref)
{
    ...
    item.varsize = 0;
    count = ntfs_invalidate_cache(vol->nidata_cache, GENERIC(&item), idata_cache_compare, CACHE_FREE);
}
...
int ntfs_inode_close(ntfs_inode *ni)
{
    ...
    if (ni)
    {
        ...
        if (ni->vol && ni->vol->nidata_cache && ((ni->mft_no == FILE_root) || ((ni->mft_no >= FILE_first_user)
                && !(ni->mrec->flags & MFT_RECORD_IS_4))))
        {
            ...
            if (dirty)
            {
                ...
                if (res) ntfs_inode_real_close(ni);
            }
            else res = 0;
            ...
            if (!res)
            {
                ...
                ntfs_enter_cache(ni->vol->nidata_cache, GENERIC(&item), idata_cache_compare);
            }
        }
        else
        {
...
ntfs_inode *ntfs_extent_inode_open(ntfs_inode *base_ni, const MFT_REF mref)
{
    ...
    if (!ni) goto out;
    if (ntfs_file_record_read(base_ni->vol, le64_to_cpu(mref), &ni->mrec, NULL)) goto err_out;
    ni->mft_no = mft_no;
    ni->nr_extents = -1;
    ni->base_ni = base_ni;
...
int ntfs_inode_attach_all_extents(ntfs_inode *ni)
{
    ...
    {
        if (ni->mft_no != MREF_LE(ale->mft_reference) && prev_attached != MREF_LE(ale->mft_reference))
        {
...
static int ntfs_inode_sync_standard_information(ntfs_inode *ni)
{
    ...
    if (test_nino_flag(ni, v3_Extensions) && (lth <= sizeof(STANDARD_INFORMATION))) ntfs_log_error("bad sync of standard information\n");
...
static int ntfs_inode_sync_file_name(ntfs_inode *ni, ntfs_inode *dir_ni)
{
    ...
    {
        if (!ntfs_attr_lookup(AT_REPARSE_POINT, NULL, 0, CASE_SENSITIVE, 0, NULL, 0, ctx))
        {
            rpp = (REPARSE_POINT*) ((u8 *) ctx->attr + le16_to_cpu(ctx->attr->value_offset));
    ...
    while (!ntfs_attr_lookup(AT_FILE_NAME, NULL, 0, 0, 0, NULL, 0, ctx))
    {
        ...
        if (MREF_LE(fn->parent_directory) == ni->mft_no)
        {
            ...
            index_ni = dir_ni;
        else index_ni = ntfs_inode_open(ni->vol, le64_to_cpu(fn->parent_directory));
        if (!index_ni)
        {
            if (!err) err = errno;
        ...
        if (!ictx)
        {
            ...
            if ((ni != index_ni) && !dir_ni && ntfs_inode_close(index_ni) && !err) err = errno;
            continue;
        }
        if (ntfs_index_lookup(fn, sizeof(FILE_NAME_ATTR), ictx))
        {
            ...
            if (ni != index_ni && ntfs_inode_close(index_ni) && !err) err = errno;
            continue;
        }
        /* Update flags and file size. */
        fnx = (FILE_NAME_ATTR *) ictx->data;
        fnx->file_attributes = (fnx->file_attributes & ~FILE_ATTR_VALID_FLAGS) | (ni->flags & FILE_ATTR_VALID_FLAGS);
        if (ni->mrec->flags & MFT_RECORD_IS_DIRECTORY)
            fnx->data_size = fnx->allocated_size = const_cpu_to_le64(0);
        else
        {
    ...
    return 0;
    err_out: if (ctx) ntfs_attr_put_search_ctx(ctx);
    errno = err;
    return -1;
}
...
static int ntfs_inode_sync_in_dir(ntfs_inode *ni, ntfs_inode *dir_ni)
{
    ...
    if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 && ntfs_inode_sync_standard_information(ni))
    {
        if (!err || errno == EIO)
        {
            err = errno;
    ...
    if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 && NInoFileNameTestAndClearDirty(ni)
    ...
    if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 && NInoAttrList(ni)
            && NInoAttrListTestAndClearDirty(ni))
    {
        ...
        {
            if (ntfs_attr_pwrite(na, 0, ni->attr_list_size, ni->attr_list) != ni->attr_list_size)
            {
                if (!err || errno == EIO)
                {
                    err = errno;
    ...
    if (ni->nr_extents > 0)
    {
        ...
        for (i = 0; i < ni->nr_extents; ++i)
        {
            ...
            eni = ni->extent_nis[i];
            if (!NInoTestAndClearDirty(eni)) continue;
...
int ntfs_inode_add_attrlist(ntfs_inode *ni)
{
    ...
    while (!ntfs_attr_lookup(AT_UNUSED, NULL, 0, 0, 0, NULL, 0, ctx))
    {
        ...
        memcpy(ale->name, (u8 *) ctx->attr + le16_to_cpu(ctx->attr->name_offset), ctx->attr->name_length
                * sizeof(ntfschar));
    ...
    if (le32_to_cpu(ni->mrec->bytes_allocated) - le32_to_cpu(ni->mrec->bytes_in_use)
            < offsetof(ATTR_RECORD, resident_end))
    {
        if (ntfs_inode_free_space(ni, offsetof(ATTR_RECORD, resident_end)))
        {
    ...
    if (ntfs_resident_attr_record_add(ni, AT_ATTRIBUTE_LIST, NULL, 0, NULL, 0, 0) < 0)
    {
        err = errno;
    ...
    {
        if (ntfs_attr_record_rm(ctx)) ntfs_log_perror("Rollback failed to remove attrlist");
    }
    else
    ...
    while ((u8*) ale < al + al_len)
    {
        ...
        {
            if (!ntfs_attr_lookup(ale->type, ale->name, ale->name_length, CASE_SENSITIVE,
                    sle64_to_cpu(ale->lowest_vcn), NULL, 0, ctx))
            {
                if (ntfs_attr_record_move_to(ctx, ni)) ntfs_log_perror("Rollback failed to "
                        "move attribute");
            }
            else
...
int ntfs_inode_free_space(ntfs_inode *ni, int size)
{
     ...
     */
    if (ntfs_attr_position(AT_FILE_NAME, ctx)) goto put_err_out;
    ...
    while (1)
    {
        ...
        {
            retry: if (ntfs_attr_position(AT_UNUSED, ctx)) goto put_err_out;
        }

        if (ntfs_inode_base(ctx->ntfs_ino)->mft_no == FILE_MFT && ctx->attr->type == AT_DATA) goto retry;

        if (ctx->attr->type == AT_INDEX_ROOT) goto retry;
         ...
         */
        ntfs_attr_reinit_search_ctx(ctx);
        if (ntfs_attr_position(AT_FILE_NAME, ctx)) break;
    }
    put_err_out: ntfs_attr_put_search_ctx(ctx);
    if (errno == ENOSPC)
...
void ntfs_inode_update_times(ntfs_inode *ni, ntfs_time_update_flags mask)
{
    ...
    if ((ni->mft_no < FILE_first_user && ni->mft_no != FILE_root) || NVolReadOnly(ni->vol) || !mask) return;

    now = ntfs_current_time();
    if (mask & NTFS_UPDATE_ATIME) ni->last_access_time = now;
    if (mask & NTFS_UPDATE_MTIME) ni->last_data_change_time = now;
    if (mask & NTFS_UPDATE_CTIME) ni->last_mft_change_time = now;
...
int ntfs_inode_badclus_bad(u64 mft_no, ATTR_RECORD *attr)
{
    ...
    if (ustr && ntfs_names_are_equal(ustr, len, (ntfschar *) ((u8 *) attr + le16_to_cpu(attr->name_offset)),
            attr->name_length, 0, NULL, 0)) ret = 1;
