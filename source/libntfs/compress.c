...
typedef enum
{
    ...
    NTFS_SYMBOL_TOKEN = 0, NTFS_PHRASE_TOKEN = 1, NTFS_TOKEN_MASK = 1,

    /* Compression sub-block constants. */
    NTFS_SB_SIZE_MASK = 0x0fff, NTFS_SB_SIZE = 0x1000, NTFS_SB_IS_COMPRESSED = 0x8000,
...
static void ntfs_new_node(struct COMPRESS_CONTEXT *pctx, unsigned int r)
{
    unsigned int pp;
    BOOL less;
    BOOL done;
    ...
    do
    {
        ...
        if (!done)
        {
            ...
            if (i >= THRESHOLD)
            {
                ...
                {
                    pctx->match_position = r - pp + 2 * NTFS_SB_SIZE - 1;
                ...
                else if ((i == pctx->match_length) && ((c = (r - pp + 2 * NTFS_SB_SIZE - 1)) < pctx->match_position)) pctx->match_position
                        = c;
            }
        }
    } while (!done);
...
static unsigned int ntfs_nextmatch(struct COMPRESS_CONTEXT *pctx, unsigned int rr, int dd)
{
    ...
    {
        rr++;
        if (pctx->match_length > 0) pctx->match_length--;
        if (!pctx->len)
        {
        ...
        if (--pctx->len)
        {
            ...
            if (((rr + bestlen) < NTFS_SB_SIZE))
            {
                ...
                if (pctx->match_length > bestlen) bestlen = pctx->match_length;
            }
            else if (dd > 0)
            {
                rr += dd;
...
static unsigned int ntfs_compress_block(const char *inbuf, unsigned int size, char *outbuf)
{
    ...
    if (pctx)
    {
        ...
        *ptag = 0;
        ...
        do
        {
            if (pctx->match_length > pctx->len) pctx->match_length = pctx->len;
            ...
            else
            {
                while ((unsigned int) (1 << pctx->nbt) <= (rr - 1))
                    pctx->nbt++;
                q = (pctx->match_position << (16 - pctx->nbt)) + pctx->match_length - THRESHOLD;
                if (ntag >= 8)
                {
                    ntag = 0;
                    ptag = &outbuf[xout++];
             ...
             * output in a loop.
             */
        } while ((pctx->len > 0) && (rr < size) && (xout < (NTFS_SB_SIZE + 2)));
        ...
        else
        {
            ...
            if (size < NTFS_SB_SIZE) memset(&outbuf[size + 2], 0, NTFS_SB_SIZE - size);
            outbuf[0] = 0xff;
            outbuf[1] = 0x3f;
            xout = NTFS_SB_SIZE + 2;
        }
 ...
 */
static int ntfs_decompress(u8 *dest, const u32 dest_size, u8 * const cb_start, const u32 cb_size)
{
    /*
    ...
    if (dest_sb_end > dest_end) goto return_overflow;
    /* Does the minimum size of a compressed sb overflow valid range? */
    if (cb + 6 > cb_end) goto return_overflow;
    /* Setup the current sub-block source pointers and validate range. */
    cb_sb_start = cb;
    cb_sb_end = cb_sb_start + (le16_to_cpup((le16*)cb) & NTFS_SB_SIZE_MASK) + 3;
    if (cb_sb_end > cb_end) goto return_overflow;
    ...
    if (!(le16_to_cpup((le16*)cb) & NTFS_SB_IS_COMPRESSED))
    {
        ...
        if (cb_sb_end - cb != NTFS_SB_SIZE) goto return_overflow;
    ...
    if (cb > cb_sb_end || dest > dest_sb_end) goto return_overflow;
    ...
    for (token = 0; token < 8; token++, tag >>= 1)
    {
        ...
        if (cb >= cb_sb_end || dest > dest_sb_end) break;
         ...
         */
        if (dest == dest_sb_start) goto return_overflow;
        /*
        ...
        if (dest_back_addr < dest_sb_start) goto return_overflow;
        /* Now calculate the length of the byte sequence. */
        length = (pt & (0xfff >> lg)) + 3;
        /* Verify destination is in range. */
        if (dest + length > dest_sb_end) goto return_overflow;
    ...
    goto do_next_tag;
    return_overflow: errno = EOVERFLOW;
 ...
 */
static BOOL ntfs_is_cb_compressed(ntfs_attr *na, runlist_element *rl, VCN cb_start_vcn, int cb_clusters)
{
    /*
     ...
     * compressed.
     */
    restart: cb_clusters -= rl->length - (cb_start_vcn - rl->vcn);
    while (cb_clusters > 0)
    {
        /* Go to the next run. */
        rl++;
...
s64 ntfs_compressed_attr_pread(ntfs_attr *na, s64 pos, s64 count, void *b)
{
    ...
    if (!na || !na->ni || !na->ni->vol || !b || ((data_flags & ATTR_COMPRESSION_MASK) != ATTR_IS_COMPRESSED) || pos < 0
            || count < 0)
    {
        errno = EINVAL;
        return -1;
    ...
    if (!NAttrNonResident(na)) return ntfs_attr_pread(na, pos, count, b);
    total = total2 = 0;
     ...
     * decompress.
     */
    end_vcn = ((pos + count + cb_size - 1) & ~cb_size_mask) >> vol->cluster_size_bits;
    /* Number of compression blocks (cbs) in the wanted vcn range. */
    nr_cbs = (end_vcn - start_vcn) << vol->cluster_size_bits >> na->compression_block_size_bits;
    cb_end = cb + cb_size;
    do_next_cb: nr_cbs--;
    cb_pos = cb;
    vcn = start_vcn;
    ...
    else
    {
        ...
        do
        {
            br = ntfs_attr_pread(na, (vcn << vol->cluster_size_bits) + (cb_pos - cb), to_read, cb_pos);
            if (br <= 0)
            {
                if (!br)
                {
        ...
        na->data_flags = data_flags;
        /* Just a precaution. */
        if (cb_pos + 2 <= cb_end) *(u16*) cb_pos = 0;
...
static u32 read_clusters(ntfs_volume *vol, const runlist_element *rl, s64 offs, u32 to_read, char *inbuf)
{
    u32 count;
    int xgot;
    u32 got;
    ...
    do
    {
        ...
        if ((to_read - got) < count) count = to_read - got;
...
static s32 write_clusters(ntfs_volume *vol, const runlist_element *rl, s64 offs, s32 to_write, const char *outbuf)
{
    s32 count;
    s32 put, xput;
    s64 xpos;
    ...
    do
    {
        ...
        if ((to_write - put) < count) count = to_write - put;
...
static s32 ntfs_comp_set(ntfs_attr *na, runlist_element *rl, s64 offs, u32 insz, const char *inbuf)
{
    ntfs_volume *vol;
    char *outbuf;
    char *pbuf;
    ...
    outbuf = (char*) ntfs_malloc(na->compression_block_size + 2 * (na->compression_block_size / NTFS_SB_SIZE) + 2);
    if (outbuf)
    {
        fail = FALSE;
        compsz = 0;
        ...
        for (p = 0; (p < insz) && !fail; p += NTFS_SB_SIZE)
        {
            ...
            if (!sz || ((compsz + sz + clsz + 2) > na->compression_block_size))
                fail = TRUE;
            else
            {
                if (allzeroes)
...
static BOOL valid_compressed_run(ntfs_attr *na, runlist_element *rl, BOOL fullcheck, const char *text)
{
    runlist_element *xrl;
    const char *err;
    BOOL ok = TRUE;
    ...
    while (xrl->length)
    {
        if ((xrl->vcn + xrl->length) != xrl[1].vcn) err = "Runs not adjacent";
        if (xrl->lcn == LCN_HOLE)
        {
            if ((xrl->vcn + xrl->length) & (na->compression_block_clusters - 1))
            {
                err = "Invalid hole";
            }
...
static int ntfs_compress_overwr_free(ntfs_attr *na, runlist_element *rl, s32 usedcnt, s32 freecnt, VCN *update_from)
{
    BOOL beginhole;
    BOOL mergeholes;
    s32 oldlength;
    ...
    if (!mergeholes && (usedcnt || beginhole))
    {
        ...
        if (freelength)
        {
            res = ntfs_cluster_free_basic(vol, freelcn, (threeparts ? freecnt : freelength));
            if (!res) freed += (threeparts ? freecnt : freelength);
            if (!usedcnt)
            {
                holes++;
                freerl--;
                freerl->length += (threeparts ? freecnt : freelength);
                if (freerl->vcn < *update_from) *update_from = freerl->vcn;
            }
        }
        while (!res && frl->length && (freed < freecnt))
        {
            ...
            {
                res = ntfs_cluster_free_basic(vol, frl->lcn, frl->length);
                if (!res)
                {
                    freed += frl->length;
                    frl->lcn = LCN_HOLE;
            ...
            {
                res = ntfs_cluster_free_basic(vol, frl->lcn, freecnt - freed);
                if (!res)
                {
            ...
            case 0:
                ...
                if (freerl->lcn == LCN_HOLE)
                {
                    if (threeparts)
                    {
                        ...
                        freerl[2].vcn = freerl[1].vcn + freerl[1].length;
                        freerl->vcn = freevcn;
                    }
                    else
                    {
...
static int ntfs_compress_free(ntfs_attr *na, runlist_element *rl, s64 used, s64 reserved, BOOL appending,
        VCN *update_from)
{
    s32 freecnt;
    s32 usedcnt;
    ...
    if (rl->vcn < *update_from) *update_from = rl->vcn;
    ...
    if (rl->length)
    {
        ...
        if (!((freevcn + freecnt) & (na->compression_block_clusters - 1)))
        {
            if (!appending)
                res = ntfs_compress_overwr_free(na, rl, usedcnt, freecnt, update_from);
            else
            {
                freelength = rl->length - usedcnt;
                beginhole = !usedcnt && !rl->vcn;
                ...
                if ((freelength > 0) && !mergeholes && (usedcnt || beginhole))
                {
                    /*
                ...
                else
                {
                    ...
                    if ((freelength > 0) && !mergeholes && !usedcnt)
                    {
                        freerl--;
                        freerl->length = freelength;
                        if (freerl->vcn < *update_from) *update_from = freerl->vcn;
                ...
                if (!res)
                {
                    na->compressed_size -= freecnt << vol->cluster_size_bits;
                    if (mergeholes)
                    {
...
static int ntfs_read_append(ntfs_attr *na, const runlist_element *rl, s64 offs, u32 compsz, s32 pos, BOOL appending,
        char *outbuf, s64 to_write, const void *b)
{
    ...
    else
    {
        ...
        if (compbuf)
        {
            ...
            else decompsz = na->compression_block_size;
            got = read_clusters(na->ni->vol, rl, offs, compsz, compbuf);
            if ((got == compsz) && !ntfs_decompress((u8*) outbuf, decompsz, (u8*) compbuf, compsz))
            {
                memcpy(&outbuf[pos], b, to_write);
                fail = 0;
            }
...
static int ntfs_flush(ntfs_attr *na, runlist_element *rl, s64 offs, const char *outbuf, s32 count, BOOL compress,
        BOOL appending, VCN *update_from)
{
    ...
    if (compress)
    {
        ...
        if (written == -1) compress = FALSE;
        if ((written >= 0) && ntfs_compress_free(na, rl, offs + written, offs + na->compression_block_size, appending,
                update_from)) written = -1;
    }
    else written = 0;
    if (!compress)
    {
        ...
        written = write_clusters(na->ni->vol, rl, offs, rounded, outbuf);
        if (written != rounded) written = -1;
    }
    return (written);
}
...
s64 ntfs_compressed_pwrite(ntfs_attr *na, runlist_element *wrl, s64 wpos, s64 offs, s64 to_write, s64 rounded,
        const void *b, int compressed_part, VCN *update_from)
{
    ...
    if ((*update_from < 0) || (compressed_part < 0) || (compressed_part > (int) na->compression_block_clusters))
    {
    ...
    if (wrl->vcn < *update_from) *update_from = wrl->vcn;
    written = -1; /* default return */
    vol = na->ni->vol;
     ...
     */
    nextblock = ((offs + (wrl->vcn << vol->cluster_size_bits)) | (na->compression_block_size - 1)) + 1;
    ...
    if (endwrite >= nextblock)
    {
        ...
        to_write = rounded = nextblock - (offs + (wrl->vcn << vol->cluster_size_bits));
    }
    start_vcn = 0;
    fail = FALSE;
    brl = wrl;
    ...
    if (compress || compressed_part)
    {
        ...
        start_vcn = (wrl->vcn + (offs >> vol->cluster_size_bits)) & -compression_length;
        if (start_vcn < *update_from) *update_from = start_vcn;
    ...
    if (compressed_part && !fail)
    {
        ...
        if (outbuf)
        {
            ...
            {
                to_read = na->data_size - (brl->vcn << vol->cluster_size_bits);
                if (to_read > na->compression_block_size) to_read = na->compression_block_size;
                to_flush = to_read;
            }
            if (!ntfs_read_append(na, brl, roffs, compsz, (s32) (offs - roffs), appending, outbuf, to_write, b))
            {
                written = ntfs_flush(na, brl, roffs, outbuf, to_flush, compress, appending, update_from);
                if (written >= 0)
                {
                    written = to_write;
                    done = TRUE;
    ...
    else
    {
        if (compress && !fail)
        {
            ...
            if (inbuf)
            {
                ...
                if (got == to_read)
                {
                    ...
                    written = ntfs_comp_set(na, brl, roffs, to_read + to_write, inbuf);
                    /*
                    ...
                    && !ntfs_compress_free(na, brl, written + roffs, na->compression_block_size + roffs, appending,
                            update_from))
                    {
                        done = TRUE;
                        written = to_write;
        ...
        if (!done)
        {
            ...
            if ((wpos + rounded) > ((wrl->lcn + wrl->length) << vol->cluster_size_bits))
            {
            ...
            {
                written = ntfs_pwrite(vol->dev, wpos, rounded, b);
                if (written == rounded) written = to_write;
            }
        }
    }
    if ((written >= 0) && !valid_compressed_run(na, wrl, TRUE, "end compressed write")) written = -1;
    return (written);
}
...
int ntfs_compressed_close(ntfs_attr *na, runlist_element *wrl, s64 offs, VCN *update_from)
{
    ntfs_volume *vol;
    runlist_element *brl; /* entry containing the beginning of block */
    int compression_length;
    ...
    if (inbuf)
    {
        ...
        if (!fail)
        {
            ...
            if (to_read)
            {
                ...
                if (got == to_read)
                {
                    written = ntfs_comp_set(na, brl, roffs, to_read, inbuf);
                    if ((written >= 0)
                    /* free the unused clusters */
                    && !ntfs_compress_free(na, brl, written + roffs, na->compression_block_size + roffs, TRUE,
                            update_from))
                    {
                        done = TRUE;
                    }
    ...
    if (done && !valid_compressed_run(na, wrl, TRUE, "end compressed close")) done = FALSE;
    return (!done);
}
