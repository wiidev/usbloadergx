...
static int ntfs_ib_write(ntfs_index_context *icx, INDEX_BLOCK *ib)
{
    ...
    ret = ntfs_attr_mst_pwrite(icx->ia_na, ntfs_ib_vcn_to_pos(icx, vcn), 1, icx->block_size, ib);
    if (ret != 1)
    {
...
{
    if (ntfs_ib_write(icx, icx->ib)) return STATUS_ERROR;
 ...
 */
ntfs_index_context *ntfs_index_ctx_get(ntfs_inode *ni, ntfschar *name, u32 name_len)
{
...
static void ntfs_index_ctx_free(ntfs_index_context *icx)
{
    ...
    if (icx->actx) ntfs_attr_put_search_ctx(icx->actx);
...
static INDEX_ENTRY *ntfs_ie_dup(INDEX_ENTRY *ie)
{
    ...
    if (dup) memcpy(dup, ie, le16_to_cpu(ie->length));
...
static INDEX_ENTRY *ntfs_ie_dup_novcn(INDEX_ENTRY *ie)
{
    ...
    if (ie->ie_flags & INDEX_ENTRY_NODE) size -= sizeof(VCN);
...
static INDEX_ROOT *ntfs_ir_lookup(ntfs_inode *ni, ntfschar *name, u32 name_len, ntfs_attr_search_ctx **ctx)
{
    ATTR_RECORD *a;
    INDEX_ROOT *ir = NULL;
 ...
 */
static int ntfs_ie_lookup(const void *key, const int key_len, ntfs_index_context *icx, INDEX_HEADER *ih, VCN *vcn,
        INDEX_ENTRY **ie_out)
{
    INDEX_ENTRY *ie;
    u8 *index_end;
    ...
    {
        /* Bounds checks. */
        if ((u8 *) ie + sizeof(INDEX_ENTRY_HEADER) > index_end || (u8 *) ie + le16_to_cpu(ie->length) > index_end)
        {
            errno = ERANGE;
        ...
        rc = icx->collate(icx->ni->vol, key, key_len, &ie->key, le16_to_cpu(ie->key_length));
...
static int ntfs_ib_read(ntfs_index_context *icx, VCN vcn, INDEX_BLOCK *dst)
{
    ...
    if (ntfs_ia_check(icx, dst, vcn)) return -1;
...
int ntfs_index_lookup(const void *key, const int key_len, ntfs_index_context *icx)
{
    ...
    else icx->vcn_size_bits = ni->vol->sector_size_bits;
    ...
    {
        err = errno;
        if (ret == STATUS_ERROR) goto err_out;
...
static INDEX_BLOCK *ntfs_ib_alloc(VCN ib_vcn, u32 ib_size, INDEX_HEADER_FLAGS node_type)
{
    INDEX_BLOCK *ib;
...
static int ntfs_ibm_add(ntfs_index_context *icx)
{
    ...
    if (ntfs_attr_exist(icx->ni, AT_BITMAP, icx->name, icx->name_len)) return STATUS_OK;
    /*
...
static VCN ntfs_ibm_get_free(ntfs_index_context *icx)
{
    ...
    bm = ntfs_attr_readall(icx->ni, AT_BITMAP, icx->name, icx->name_len, &size);
    if (!bm) return (VCN) -1;

    for (byte = 0; byte < size; byte++)
    {

        if (bm[byte] == 255) continue;
    ...
    if (ntfs_ibm_set(icx, vcn)) vcn = (VCN) -1;
...
static int ntfs_ib_copy_tail(ntfs_index_context *icx, INDEX_BLOCK *src, INDEX_ENTRY *median, VCN new_vcn)
{
    u8 *ies_end;
    ...
    dst = ntfs_ib_alloc(new_vcn, icx->block_size, src->index.ih_flags & NODE_MASK);
    if (!dst) return STATUS_ERROR;
...
static int ntfs_ib_cut_tail(ntfs_index_context *icx, INDEX_BLOCK *ib, INDEX_ENTRY *ie)
{
    char *ies_start, *ies_end;
    INDEX_ENTRY *ie_last;
    ...
    ie_last = ntfs_ie_get_last((INDEX_ENTRY *) ies_start, ies_end);
    if (ie_last->ie_flags & INDEX_ENTRY_NODE) ntfs_ie_set_vcn(ie_last, ntfs_ie_get_vcn(ie));
    ...
    if (ntfs_ib_write(icx, ib)) return STATUS_ERROR;
...
static int ntfs_ia_add(ntfs_index_context *icx)
{
    ...
    if (ntfs_ibm_add(icx)) return -1;

    if (!ntfs_attr_exist(icx->ni, AT_INDEX_ALLOCATION, icx->name, icx->name_len))
    {

        if (ntfs_attr_add(icx->ni, AT_INDEX_ALLOCATION, icx->name, icx->name_len, NULL, 0))
        {
...
static int ntfs_ir_reparent(ntfs_index_context *icx)
{
    ...
    if ((ir->index.ih_flags & NODE_MASK) == SMALL_INDEX) if (ntfs_ia_add(icx)) goto out;

    new_ib_vcn = ntfs_ibm_get_free(icx);
    if (new_ib_vcn == -1) goto out;

    ir = ntfs_ir_lookup2(icx->ni, icx->name, icx->name_len);
    if (!ir) goto clear_bmp;
    ...
    if (ntfs_ib_write(icx, ib)) goto clear_bmp;
    ...
    if (ntfs_resident_attr_value_resize(ctx->mrec, ctx->attr, sizeof(INDEX_ROOT) - sizeof(INDEX_HEADER)
    ...
    ntfs_attr_put_search_ctx(ctx);
    out: return ret;
    clear_bmp: ntfs_ibm_clear(icx, new_ib_vcn);
    goto err_out;
}
...
static int ntfs_ir_truncate(ntfs_index_context *icx, int data_size)
{
    ...
    if (ret == STATUS_OK)
    {
        ...
        if (!icx->ir) return STATUS_ERROR;

        icx->ir->index.allocated_size = cpu_to_le32(data_size);

    }
    else if (ret == STATUS_ERROR) ntfs_log_perror("Failed to truncate INDEX_ROOT");
...
static int ntfs_ih_insert(INDEX_HEADER *ih, INDEX_ENTRY *orig_ie, VCN new_vcn, int pos)
{
    ...
    if (!ie) return STATUS_ERROR;

    if (!(ie->ie_flags & INDEX_ENTRY_NODE)) if (ntfs_ie_add_vcn(&ie)) goto out;
...
static int ntfs_ir_insert_median(ntfs_index_context *icx, INDEX_ENTRY *median, VCN new_vcn)
{
    u32 new_size;
    int ret;
    ...
    new_size = le32_to_cpu(icx->ir->index.index_length) + le16_to_cpu(median->length);
    if (!(median->ie_flags & INDEX_ENTRY_NODE)) new_size += sizeof(VCN);
    ...
    return ntfs_ih_insert(&icx->ir->index, median, new_vcn, ntfs_icx_parent_pos(icx));
}
...
static int ntfs_ib_insert(ntfs_index_context *icx, INDEX_ENTRY *ie, VCN new_vcn)
{
    ...
    if (ntfs_ib_read(icx, old_vcn, ib)) goto err_out;
    ...
    if (idx_size + le16_to_cpu(ie->length) + sizeof(VCN) > allocated_size)
    {
        ...
        if (err == STATUS_OK) err = STATUS_KEEP_SEARCHING;
        goto err_out;
    }

    if (ntfs_ih_insert(&ib->index, ie, new_vcn, ntfs_icx_parent_pos(icx))) goto err_out;

    if (ntfs_ib_write(icx, ib)) goto err_out;
...
static int ntfs_ib_split(ntfs_index_context *icx, INDEX_BLOCK *ib)
{
    ...
    if (ntfs_icx_parent_dec(icx)) return STATUS_ERROR;

    median = ntfs_ie_get_median(&ib->index);
    new_vcn = ntfs_ibm_get_free(icx);
    if (new_vcn == -1) return STATUS_ERROR;
    ...
    else ret = ntfs_ib_insert(icx, median, new_vcn);
...
int ntfs_ie_add(ntfs_index_context *icx, INDEX_ENTRY *ie)
{
    ...
    while (1)
    {
        ...
        if (icx->is_in_root)
            ih = &icx->ir->index;
        else ih = &icx->ib->index;

        allocated_size = le32_to_cpu(ih->allocated_size);
        new_size = le32_to_cpu(ih->index_length) + le16_to_cpu(ie->length);

        if (new_size <= allocated_size) break;
        ...
        if (icx->is_in_root)
        {
            if (ntfs_ir_make_space(icx, new_size) == STATUS_ERROR) goto err_out;
        }
        else
        {
            if (ntfs_ib_split(icx, icx->ib) == STATUS_ERROR) goto err_out;
        }
...
static int ntfs_ih_takeout(ntfs_index_context *icx, INDEX_HEADER *ih, INDEX_ENTRY *ie, INDEX_BLOCK *ib)
{
    INDEX_ENTRY *ie_roam;
    int ret = STATUS_ERROR;
    ...
    if (!ie_roam) return STATUS_ERROR;

    ntfs_ie_delete(ih, ie);

    if (ntfs_icx_parent_vcn(icx) == VCN_INDEX_ROOT_PARENT)
        ntfs_inode_mark_dirty(icx->actx->ntfs_ino);
    else if (ntfs_ib_write(icx, ib)) goto out;
 ...
 */
static int ntfs_ih_reparent_end(ntfs_index_context *icx, INDEX_HEADER *ih, INDEX_BLOCK *ib)
{
...
static int ntfs_index_rm_leaf(ntfs_index_context *icx)
{
    ...
    if (ntfs_ibm_clear(icx, icx->parent_vcn[icx->pindex + 1])) return STATUS_ERROR;
    ...
    {
        ib = ntfs_malloc(icx->block_size);
        if (!ib) return STATUS_ERROR;

        if (ntfs_ib_read(icx, ntfs_icx_parent_vcn(icx), ib)) goto out;
    ...
    if (ntfs_ih_reparent_end(icx, parent_ih, ib)) goto out;
    ok: ret = STATUS_OK;
    out: free(ib);
    return ret;
}
...
static int ntfs_index_rm_node(ntfs_index_context *icx)
{
    ...
    {
        icx->ia_na = ntfs_ia_open(icx, icx->ni);
        if (!icx->ia_na) return STATUS_ERROR;
    }
    ...
    pindex = icx->pindex;
    descend: vcn = ntfs_ie_get_vcn(ie_succ);
    if (ntfs_ib_read(icx, vcn, ib)) goto out;

    ie_succ = ntfs_ie_get_first(&ib->index);

    if (ntfs_icx_parent_inc(icx)) goto out;

    icx->parent_vcn[icx->pindex] = vcn;
    icx->parent_pos[icx->pindex] = 0;

    if ((ib->index.ih_flags & NODE_MASK) == INDEX_NODE) goto descend;
    ...
    if (ntfs_ie_add_vcn(&ie)) goto out2;
    ...
    if (delta > 0)
    {
        ...
        else if (new_size > le32_to_cpu(ih->allocated_size))
        {
            ...
            if (ret == STATUS_OK) ret = STATUS_KEEP_SEARCHING;
            goto out2;
        }
    }
    ...
    if (icx->is_in_root)
    {
        if (ntfs_ir_truncate(icx, new_size)) goto out2;
    }
    else if (ntfs_icx_ib_write(icx)) goto out2;

    ntfs_ie_delete(&ib->index, ie_succ);

    if (ntfs_ih_zero_entry(&ib->index))
    {
        if (ntfs_index_rm_leaf(icx)) goto out2;
    }
    else if (ntfs_ib_write(icx, ib)) goto out2;
...
int ntfs_index_rm(ntfs_index_context *icx)
{
    ...
    else if (icx->is_in_root || !ntfs_ih_one_entry(ih))
    {
        ...
        if (icx->is_in_root)
        {
            ...
            if (err != STATUS_OK) goto err_out;
        }
        else if (ntfs_icx_ib_write(icx)) goto err_out;
    }
    else
    {
        if (ntfs_index_rm_leaf(icx)) goto err_out;
    }
    out: return ret;
    err_out: ret = STATUS_ERROR;
    goto out;
}

int ntfs_index_remove(ntfs_inode *dir_ni, ntfs_inode *ni, const void *key, const int keylen)
{
    int ret = STATUS_ERROR;
    ntfs_index_context *icx;
    ...
    while (1)
    {
        ...
        if (ntfs_index_lookup(key, keylen, icx)) goto err_out;

        if ((((FILE_NAME_ATTR *) icx->data)->file_attributes & FILE_ATTR_REPARSE_POINT) && !ntfs_possible_symlink(ni))
        {
            errno = EOPNOTSUPP;
            goto err_out;
        }
...
INDEX_ROOT *ntfs_index_root_get(ntfs_inode *ni, ATTR_RECORD *attr)
{
    ...
    if (!ntfs_ir_lookup(ni, name, attr->name_length, &ctx)) return NULL;

    root = ntfs_malloc(sizeof(INDEX_ROOT));
    if (!root) goto out;

    *root = *((INDEX_ROOT *) ((u8 *) ctx->attr + le16_to_cpu(ctx->attr->value_offset)));
    out: ntfs_attr_put_search_ctx(ctx);
    return root;
}
...
static INDEX_ENTRY *ntfs_index_walk_down(INDEX_ENTRY *ie, ntfs_index_context *ictx)
{
    INDEX_ENTRY *entry;
    s64 vcn;
...
static INDEX_ENTRY *ntfs_index_walk_up(INDEX_ENTRY *ie, ntfs_index_context *ictx)
{
    INDEX_ENTRY *entry;
    s64 vcn;
    ...
    if (ictx->pindex > 0)
    {
        do
        {
            ...
            if (!ictx->pindex)
            {
                ...
                if (ictx->actx) free(ictx->actx);
                ictx->ir = ntfs_ir_lookup(ictx->ni, ictx->name, ictx->name_len, &ictx->actx);
                if (ictx->ir)
                    entry = ntfs_ie_get_by_pos(&ictx->ir->index, ictx->parent_pos[ictx->pindex]);
                else entry = (INDEX_ENTRY*) NULL;
            }
            else
            {
                ...
                if (!ntfs_ib_read(ictx, vcn, ictx->ib))
                {
                    entry = ntfs_ie_get_by_pos(&ictx->ib->index, ictx->parent_pos[ictx->pindex]);
                }
                else entry = (INDEX_ENTRY*) NULL;
            }
            ictx->entry = entry;
        } while (entry && (ictx->pindex > 0) && (entry->ie_flags & INDEX_ENTRY_END));
    }
    else entry = (INDEX_ENTRY*) NULL;
    return (entry);
}
...
INDEX_ENTRY *ntfs_index_next(INDEX_ENTRY *ie, ntfs_index_context *ictx)
{
    ...
    if (next && (next->ie_flags & INDEX_ENTRY_END)) next = (INDEX_ENTRY*) NULL;
    return (next);
}
