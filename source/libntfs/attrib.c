...
ntfschar STREAM_SDS[] = { const_cpu_to_le16('$'), const_cpu_to_le16('S'), const_cpu_to_le16('D'),
        const_cpu_to_le16('S'), const_cpu_to_le16('\0') };

ntfschar TXF_DATA[] = { const_cpu_to_le16('$'), const_cpu_to_le16('T'), const_cpu_to_le16('X'), const_cpu_to_le16('F'),
        const_cpu_to_le16('_'), const_cpu_to_le16('D'), const_cpu_to_le16('A'), const_cpu_to_le16('T'),
        const_cpu_to_le16('A'), const_cpu_to_le16('\0') };

static int NAttrFlag(ntfs_attr *na, FILE_ATTR_FLAGS flag)
{
    if (na->type == AT_DATA && na->name == AT_UNNAMED) return (na->ni->flags & flag);
    return 0;
}
...
static void NAttrSetFlag(ntfs_attr *na, FILE_ATTR_FLAGS flag)
{
    if (na->type == AT_DATA && na->name == AT_UNNAMED)
        na->ni->flags |= flag;
else    ntfs_log_trace("Denied setting flag %d for not unnamed data "
            "attribute\n", flag);
}

static void NAttrClearFlag(ntfs_attr *na, FILE_ATTR_FLAGS flag)
{
    if (na->type == AT_DATA && na->name == AT_UNNAMED) na->ni->flags &= ~flag;
}
...
s64 ntfs_get_attribute_value_length(const ATTR_RECORD *a)
{
    ...
    errno = 0;
    if (a->non_resident) return sle64_to_cpu(a->data_size);
 ...
 * Returns:
 */
s64 ntfs_get_attribute_value(const ntfs_volume *vol, const ATTR_RECORD *a, u8 *b)
{
    runlist *rl;
    s64 total, r;
    int i;
    ...
    if (!a->non_resident)
    {
        ...
        /* Sanity check. */
        if (le32_to_cpu(a->value_length) + le16_to_cpu(a->value_offset) > le32_to_cpu(a->length))
        {
            return 0;
        }

        memcpy(b, (const char*) a + le16_to_cpu(a->value_offset), le32_to_cpu(a->value_length));
        errno = 0;
        return (s64) le32_to_cpu(a->value_length);
    }
    ...
    {
        if (total + (rl[i].length << vol->cluster_size_bits) >= sle64_to_cpu(a->data_size))
        {
             ...
             */
            r = ntfs_pread(vol->dev, rl[i].lcn << vol->cluster_size_bits, rl[i].length << vol->cluster_size_bits,
                    intbuf);
...
#undef ESTR
            ...
            memcpy(b + total, intbuf, sle64_to_cpu(a->data_size) - total);
            free(intbuf);
         ...
         */
        r
                = ntfs_pread(vol->dev, rl[i].lcn << vol->cluster_size_bits, rl[i].length << vol->cluster_size_bits, b
                        + total);
        if (r != rl[i].length << vol->cluster_size_bits)
        {
#define ESTR "Error reading attribute value"
 ...
 */
static void __ntfs_attr_init(ntfs_attr *na, ntfs_inode *ni, const ATTR_TYPES type, ntfschar *name, const u32 name_len)
{
    na->rl = NULL;
    na->ni = ni;
    na->type = type;
 ...
 */
void ntfs_attr_init(ntfs_attr *na, const BOOL non_resident, const ATTR_FLAGS data_flags, const BOOL encrypted,
        ...
        const s64 compressed_size, const u8 compression_unit)
{
    if (!NAttrInitialized(na))
    {
        ...
        if (non_resident) NAttrSetNonResident(na);
        if (data_flags & ATTR_COMPRESSION_MASK) NAttrSetCompressed(na);
        if (encrypted) NAttrSetEncrypted(na);
        if (sparse) NAttrSetSparse(na);
        ...
        if ((data_flags & ATTR_COMPRESSION_MASK) || sparse)
        {
            ...
            na->compression_block_size = 1 << (compression_unit + vol->cluster_size_bits);
            na->compression_block_size_bits = ffs(na->compression_block_size) - 1;
        }
...
ntfs_attr *ntfs_attr_open(ntfs_inode *ni, const ATTR_TYPES type, ntfschar *name, u32 name_len)
{
    ...
    if (ntfs_attr_lookup(type, name, name_len, 0, 0, NULL, 0, ctx)) goto put_err_out;
     ...
     * does not detect or fix them so we need to cope with it, too.
     */
    if (type == AT_ATTRIBUTE_LIST) a->flags = 0;
    ...
    if ((type == AT_DATA) && !a->initialized_size)
    {
        ...
        if ((ni->flags & FILE_ATTR_COMPRESSED) && (ni->vol->major_ver >= 3) && NVolCompression(ni->vol)
                && (ni->vol->cluster_size <= MAX_COMPRESSION_CLUSTER_SIZE)) a->flags |= ATTR_IS_COMPRESSED;
    }

    cs = a->flags & (ATTR_IS_COMPRESSED | ATTR_IS_SPARSE);

    if (na->type == AT_DATA && na->name == AT_UNNAMED && ((!(a->flags & ATTR_IS_SPARSE) != !NAttrSparse(na))
    ...
    if (a->non_resident)
    {
        if ((a->flags & ATTR_COMPRESSION_MASK) && !a->compression_unit)
        {
            errno = EIO;
        ...
        ntfs_attr_init(na, TRUE, a->flags, a->flags & ATTR_IS_ENCRYPTED, a->flags & ATTR_IS_SPARSE,
                sle64_to_cpu(a->allocated_size), sle64_to_cpu(a->data_size), sle64_to_cpu(a->initialized_size),
                cs ? sle64_to_cpu(a->compressed_size) : 0, cs ? a->compression_unit : 0);
    }
    else
    {
        s64 l = le32_to_cpu(a->value_length);
        ntfs_attr_init(na, FALSE, a->flags, a->flags & ATTR_IS_ENCRYPTED, a->flags & ATTR_IS_SPARSE, (l + 7) & ~7, l,
                l, cs ? (l + 7) & ~7 : 0, 0);
    }
    ...
    put_err_out: ntfs_attr_put_search_ctx(ctx);
    err_out: free(newname);
    free(na);
    na = NULL;
    goto out;
}
...
{
    if (!na) return;
    if (NAttrNonResident(na) && na->rl) free(na->rl);
    /* Don't release if using an internal constant. */
    if (na->name != AT_UNNAMED && na->name != NTFS_INDEX_I30 && na->name != STREAM_SDS) free(na->name);
    free(na);
}
...
int ntfs_attr_map_runlist(ntfs_attr *na, VCN vcn)
{
    ...
    if (!ntfs_attr_lookup(na->type, na->name, na->name_len, CASE_SENSITIVE, vcn, NULL, 0, ctx))
    {
        runlist_element *rl;
...
int ntfs_attr_map_whole_runlist(ntfs_attr *na)
{
    ...
    while (1)
    {
        ...
        int not_mapped = 0;
        if (ntfs_rl_vcn_to_lcn(na->rl, next_vcn) == LCN_RL_NOT_MAPPED) not_mapped = 1;

        if (ntfs_attr_lookup(na->type, na->name, na->name_len, CASE_SENSITIVE, next_vcn, NULL, 0, ctx)) break;
        ...
        if (!next_vcn)
        {
            ...
            last_vcn = sle64_to_cpu(a->allocated_size) >> vol->cluster_size_bits;
        }
    ...
    err_out: ntfs_attr_put_search_ctx(ctx);
    out:
    ntfs_log_leave("\n");
    return ret;
}
...
LCN ntfs_attr_vcn_to_lcn(ntfs_attr *na, const VCN vcn)
{
    ...
    if (!na || !NAttrNonResident(na) || vcn < 0) return (LCN) LCN_EINVAL;
     ...
     */
    if (!is_retry || lcn == (LCN) LCN_RL_NOT_MAPPED) return (LCN) LCN_EIO;
...
static s64 ntfs_attr_pread_i(ntfs_attr *na, const s64 pos, s64 count, void *b)
{
    ...
    if (!NAttrNonResident(na))
    {
        ...
        {
            res_err_out: ntfs_attr_put_search_ctx(ctx);
            return -1;
        }
        val = (char*) ctx->attr + le16_to_cpu(ctx->attr->value_offset);
        if (val < (char*) ctx->attr || val + le32_to_cpu(ctx->attr->value_length) > (char*) ctx->mrec
                + vol->mft_record_size)
        {
            errno = EIO;
...
static int ntfs_attr_fill_zero(ntfs_attr *na, s64 pos, s64 count)
{
    ...
    while (pos < end)
    {
        while (rli->length && (ofsi + (rli->length << vol->cluster_size_bits) <= pos))
        {
...
static int ntfs_attr_fill_hole(ntfs_attr *na, s64 count, s64 *ofs, runlist_element **rl, VCN *update_from)
{
    s64 to_write;
    s64 need;
    ntfs_volume *vol = na->ni->vol;
    ...
    *rl = ntfs_attr_find_vcn(na, cur_vcn);
    ...
    while (rlc->vcn)
    {
        ...
        if (rlc->lcn >= 0)
        {
            ...
            else lcn_seek_from = rlc->lcn + (from_vcn - rlc->vcn);
            break;
        }
    }
    if (lcn_seek_from == -1)
    {
        ...
        while (rlc->length)
        {
            ...
            if (rlc->lcn >= 0)
            {
                ...
                if (lcn_seek_from < -1) lcn_seek_from = -1;
                break;
            }
        }
    }

    need = ((*ofs + to_write - 1) >> vol->cluster_size_bits) + 1 + (*rl)->vcn - from_vcn;
    if ((na->data_flags & ATTR_COMPRESSION_MASK) && (need < na->compression_block_clusters))
    {
        /*
        ...
        if ((from_vcn & -na->compression_block_clusters) <= (*rl)->vcn)
            alloc_vcn = (*rl)->vcn;
        else alloc_vcn = from_vcn & -na->compression_block_clusters;
        need = (alloc_vcn | (na->compression_block_clusters - 1)) + 1 - alloc_vcn;
        if (need > (*rl)->length)
        {
        ...
        rlc = ntfs_cluster_alloc(vol, alloc_vcn, need, lcn_seek_from, DATA_ZONE);
    }
    else rlc = ntfs_cluster_alloc(vol, from_vcn, need, lcn_seek_from, DATA_ZONE);
    if (!rlc) goto err_out;
    if (na->data_flags & (ATTR_COMPRESSION_MASK | ATTR_IS_SPARSE)) na->compressed_size += need
    ...
    *rl = ntfs_runlists_merge(na->rl, rlc);
    ...
    na->unused_runs = 2;
    na->rl = *rl;
    if ((*update_from == -1) || (from_vcn < *update_from)) *update_from = from_vcn;
    *rl = ntfs_attr_find_vcn(na, cur_vcn);
    if (!*rl)
    {
        /*
    ...
    if (*ofs)
    {
        ...
        if (ntfs_attr_fill_zero(na, cur_vcn << vol->cluster_size_bits, *ofs)) goto err_out;
    }
    if ((*rl)->vcn < cur_vcn)
    {
        /*
...
static int split_compressed_hole(ntfs_attr *na, runlist_element **prl, s64 pos, s64 count, VCN *update_from)
{
    int compressed_part;
    ...
    if (*prl && (rl->length > na->compression_block_clusters))
    {
        ...
        s32 endblock = (((pos + count - 1) >> cluster_size_bits) | (na->compression_block_clusters - 1)) + 1 - rl->vcn;

        compressed_part = na->compression_block_clusters - (rl->length & (na->compression_block_clusters - 1));
        if ((beginwrite + compressed_part) >= na->compression_block_clusters) compressed_part
        ...
        if (endblock < rl[0].length)
        {
            ...
            if (endblock > na->compression_block_clusters)
            {
                ...
                rl[0].length = endblock - na->compression_block_clusters;
                rl[1].lcn = LCN_HOLE;
                rl[2].lcn = LCN_HOLE;
                rl[1].vcn = rl[0].vcn + rl[0].length;
                rl[2].vcn = rl[1].vcn + na->compression_block_clusters;
                rl = ++(*prl);
            }
            else
            {
        ...
        if ((*update_from == -1) || ((*prl)->vcn < *update_from)) *update_from = (*prl)->vcn;
    }
...
static int borrow_from_hole(ntfs_attr *na, runlist_element **prl, s64 pos, s64 count, VCN *update_from,
        BOOL wasnonresident)
{
    ...
    if (undecided || nothole)
    {
         ...
         */
        irl = *prl - na->rl;
        if (!NAttrBeingNonResident(na) && ntfs_attr_map_whole_runlist(na))
        {
    ...
    if (rl && (allocated < endblock) && (zrl->lcn == LCN_HOLE))
    {
        ...
        if (*prl)
        {
            if (wasnonresident) compressed_part = na->compression_block_clusters - zrl->length;
            xofs = 0;
            if (ntfs_attr_fill_hole(na, zrl->length << cluster_size_bits, &xofs, &zrl, update_from))
                compressed_part = -1;
            else
            {
    ...
    else if ((*update_from == -1) || ((*prl)->vcn < *update_from)) *update_from = (*prl)->vcn;
    return (compressed_part);
}
...
s64 ntfs_attr_pwrite(ntfs_attr *na, const s64 pos, s64 count, const void *b)
{
    ...
    vol = na->ni->vol;
    compressed = (na->data_flags & ATTR_COMPRESSION_MASK) != const_cpu_to_le16(0);
     ...
     */
    if ((na->data_flags & ATTR_IS_ENCRYPTED) && (compressed || !vol->efs_raw))
    {
        errno = EACCES;
        goto errno_set;
    }
     ...
     */
    if (compressed && (na->type == AT_DATA) && (pos > na->initialized_size) && stuff_hole(na, pos)) goto errno_set;
     ...
     */
    if (compressed && ((na->type != AT_DATA) || ((na->data_flags & ATTR_COMPRESSION_MASK) != ATTR_IS_COMPRESSED)))
    {
        errno = EOPNOTSUPP;
        goto errno_set;
    }
    ...
    if (compressed)
    {
        ...
        if (count > fullcount) count = fullcount;
    }
    ...
    if (pos + count > na->initialized_size)
    {
        ...
        if (pos > na->initialized_size) if (ntfs_attr_fill_zero(na, na->initialized_size, pos - na->initialized_size)) goto err_out;
        ...
        if (ntfs_mft_record_write(vol, ctx->ntfs_ino->mft_no, ctx->mrec))
        {
            ...
            ntfs_mft_record_write(vol, ctx->ntfs_ino->mft_no, ctx->mrec);
            goto err_out;
        }
        na->initialized_size = pos + count;
#if CACHE_NIDATA_SIZE
        if (na->ni->mrec->flags & MFT_RECORD_IS_DIRECTORY ? na->type == AT_INDEX_ROOT && na->name == NTFS_INDEX_I30
                : na->type == AT_DATA && na->name == AT_UNNAMED)
        {
            na->ni->data_size = na->data_size;
            if ((compressed || NAttrSparse(na)) && NAttrNonResident(na))
                na->ni->allocated_size = na->compressed_size;
            else na->ni->allocated_size = na->allocated_size;
    ...
    if (compressed)
    {
        if ((rl->lcn == (LCN) LCN_HOLE) && wasnonresident)
        {
            if (rl->length < na->compression_block_clusters)
                /*
                 ...
                 * it fully
                 */
                compressed_part = na->compression_block_clusters - rl->length;
            else
            {
                /*
                 ...
                 */
                compressed_part = split_compressed_hole(na, &rl, pos, count, &update_from);
            }
        }
        else
        {
            if (rl->lcn >= 0)
            {
                 ...
                 */
                compressed_part = borrow_from_hole(na, &rl, pos, count, &update_from, wasnonresident);
            }
        }

        if (compressed_part < 0) goto err_out;

        /* just making non-resident, so not yet compressed */
        if (NAttrBeingNonResident(na) && (compressed_part < na->compression_block_clusters)) compressed_part = 0;
    }
    ofs = pos - (rl->vcn << vol->cluster_size_bits);
    /*
    ...
    for (hole_end = 0; count; rl++, ofs = 0)
    {
        ...
        if (rl->lcn < (LCN) 0)
        {
            ...
            if (ntfs_attr_fill_hole(na, fullcount, &ofs, &rl, &update_from)) goto err_out;
        }
        if (compressed)
        {
        ...
        if (!NVolReadOnly(vol))
        {
             ...
             */
            if (rounding && ((wend == na->initialized_size) || (wend < (hole_end << vol->cluster_size_bits))))
            {
                ...
                if (compressed)
                {
                    written = ntfs_compressed_pwrite(na, rl, wpos, ofs, to_write, rounding, cb, compressed_part,
                            &update_from);
                }
                else
                {
                    written = ntfs_pwrite(vol->dev, wpos, rounding, cb);
                    if (written == rounding) written = to_write;
                }
            ...
            else
            {
                if (compressed)
                {
                    written = ntfs_compressed_pwrite(na, rl, wpos, ofs, to_write, to_write, b, compressed_part,
                            &update_from);
                }
                else written = ntfs_pwrite(vol->dev, wpos, to_write, b);
            }
        }
        else written = to_write;
        /* If everything ok, update progress counters and continue. */
        if (written > 0)
        {
            total += written;
    ...
    done: if (ctx) ntfs_attr_put_search_ctx(ctx);
    /*
     ...
     */
    updatemap = (compressed ? NAttrFullyMapped(na) != 0 : update_from != -1);
    if (updatemap) if (ntfs_attr_update_mapping_pairs(na, (update_from < 0 ? 0 : update_from)))
    {
        /*
    ...
    if (total)
    {
        ...
        {
            if (pos + total > na->initialized_size) goto done;
            /*
    ...
    if (need_to.undo_initialized_size)
    {
        ...
        else ntfs_attr_reinit_search_ctx(ctx);
        if (!err)
        {
            err = ntfs_attr_lookup(na->type, na->name, na->name_len, 0, 0, NULL, 0, ctx);
            if (!err)
            {
                na->initialized_size = old_initialized_size;
                ctx->attr->initialized_size = cpu_to_sle64(
                        old_initialized_size);
                err = ntfs_mft_record_write(vol, ctx->ntfs_ino->mft_no, ctx->mrec);
            }
        }
        if (err)
        {
    ...
    /* Update mapping pairs if needed. */
    updatemap = (compressed ? NAttrFullyMapped(na) != 0 : update_from != -1);
    if (updatemap) ntfs_attr_update_mapping_pairs(na, 0);
    /* Restore original data_size if needed. */
    if (need_to.undo_data_size && ntfs_attr_truncate(na, old_data_size)) ntfs_log_perror("Failed to restore data_size");
    errno = eo;
    errno_set: total = -1;
    goto out;
}
...
int ntfs_attr_pclose(ntfs_attr *na)
{
    ...
    if (!compressed || !NAttrNonResident(na)) goto out;
    ...
    if (rl->lcn >= 0)
    {
        ...
        compressed_part = (-xrl->length) & (na->compression_block_clusters - 1);
    }
    else if (rl->lcn == (LCN) LCN_HOLE)
    {
        if (rl->length < na->compression_block_clusters)
            compressed_part = na->compression_block_clusters - rl->length;
        else compressed_part = na->compression_block_clusters;
    }
    /* done, if the last block set was compressed */
    if (compressed_part) goto out;
    ...
    if (rl->lcn < (LCN) 0)
    {
        ...
        if (ntfs_attr_fill_hole(na, (s64) 0, &ofs, &rl, &update_from)) goto err_out;
    }
    while (rl->length && (ofs >= (rl->length << vol->cluster_size_bits)))
    {
        ofs -= rl->length << vol->cluster_size_bits;
    ...
    if (failed)
    {
        ...
        if (errno == EINTR)
            goto retry;
        else goto rl_err_out;
    }
    if (ctx) ntfs_attr_put_search_ctx(ctx);
    /* Update mapping pairs if needed. */
    if (NAttrFullyMapped(na)) if (ntfs_attr_update_mapping_pairs(na, update_from))
    {
        /*
         * FIXME: trying to recover by goto rl_err_out;
         * could cause driver hang by infinite looping.
    ...
    if (ctx) ntfs_attr_put_search_ctx(ctx);
    /* Update mapping pairs if needed. */
    if (NAttrFullyMapped(na)) ntfs_attr_update_mapping_pairs(na, 0);
    errno = eo;
    errno_set: ok = FALSE;
    goto out;
}
 ...
 */
s64 ntfs_attr_mst_pread(ntfs_attr *na, const s64 pos, const s64 bk_cnt, const u32 bk_size, void *dst)
{
    s64 br;
    u8 *end;
 ...
 * achieved.
 */
s64 ntfs_attr_mst_pwrite(ntfs_attr *na, const s64 pos, s64 bk_cnt, const u32 bk_size, void *src)
{
    s64 written, i;
 ...
 */
static int ntfs_attr_find(const ATTR_TYPES type, const ntfschar *name, const u32 name_len, const IGNORE_CASE_BOOL ic,
        const u8 *val, const u32 val_len, ntfs_attr_search_ctx *ctx)
{
    ...
    else a = (ATTR_RECORD*) ((char*) ctx->attr + le32_to_cpu(ctx->attr->length));
    for (;; a = (ATTR_RECORD*) ((char*) a + le32_to_cpu(a->length)))
    {
        if (p2n(a) < p2n(ctx->mrec) || (char*) a > (char*) ctx->mrec + le32_to_cpu(ctx->mrec->bytes_allocated)) break;
        ctx->attr = a;
        ...
        {
            register int rc;
            if (name && ((rc = ntfs_names_full_collate(name, name_len, (ntfschar*) ((char*) a
                    + le16_to_cpu(a->name_offset)), a->name_length, ic, upcase, upcase_len))))
            {
                /*
        ...
        else
        {
            ...
            rc = memcmp(val, (char*) a + le16_to_cpu(a->value_offset), min(val_len,
...
char *ntfs_attr_name_get(const ntfschar *uname, const int uname_len)
{
    ...
    else if (name_len > 0) return name;
 ...
 */
static int ntfs_external_attr_find(ATTR_TYPES type, const ntfschar *name, const u32 name_len,
        const IGNORE_CASE_BOOL ic, const VCN lowest_vcn, const u8 *val, const u32 val_len, ntfs_attr_search_ctx *ctx)
{
    ...
    if (ni == base_ni) ctx->base_attr = ctx->attr;
    if (type == AT_END) goto not_found;
    vol = base_ni->vol;
    ...
    if (ctx->is_first)
    {
         ...
         * attribute.
         */
        if ((type == AT_UNUSED) && is_first_search && le32_to_cpu(al_entry->type) > le32_to_cpu(AT_ATTRIBUTE_LIST)) goto find_attr_list_attr;
    }
    else
    {
         ...
         */
        if ((type == AT_UNUSED) && le32_to_cpu(ctx->al_entry->type) < le32_to_cpu(AT_ATTRIBUTE_LIST)
    ...
    {
        /* Out of bounds check. */
        if ((u8*) al_entry < base_ni->attr_list || (u8*) al_entry > al_end) break; /* Inode is corrupt. */
        ctx->al_entry = al_entry;
        /* Catch the end of the attribute list. */
        if ((u8*) al_entry == al_end) goto not_found;
        if (!al_entry->length) break;
        ...
        if (type != AT_UNUSED)
        {
            ...
            if (type != al_entry->type) continue;
        }
         ...
         */
        if (type == AT_UNUSED) goto is_enumeration;
        /*
         * If @name is AT_UNNAMED we want an unnamed attribute.
         * If @name is present, compare the two names.
         * Otherwise, match any attribute.
        ...
        else
        {
            ...
            if (name && ((rc = ntfs_names_full_collate(name, name_len, al_name, al_name_len, ic, vol->upcase,
                    vol->upcase_len))))
            {
         ...
         */
        if (lowest_vcn && (u8*) next_al_entry >= al_start && (u8*) next_al_entry + 6 < al_end && (u8*) next_al_entry
                + le16_to_cpu(
                        next_al_entry->length) <= al_end && sle64_to_cpu(next_al_entry->lowest_vcn) <= lowest_vcn
                && next_al_entry->type == al_entry->type && next_al_entry->name_length == al_name_len
                && ntfs_names_are_equal((ntfschar*) ((char*) next_al_entry + next_al_entry->name_offset),
                        next_al_entry->name_length, al_name, al_name_len, CASE_SENSITIVE, vol->upcase, vol->upcase_len)) continue;
        is_enumeration: if (MREF_LE(al_entry->mft_reference) == ni->mft_no)
        {
            if (MSEQNO_LE(al_entry->mft_reference) != le16_to_cpu(
         ...
         */
        do_next_attr_loop: if ((char*) a < (char*) ctx->mrec || (char*) a > (char*) ctx->mrec
                + le32_to_cpu(ctx->mrec->bytes_allocated)) break;
        if (a->type == AT_END) continue;
        if (!a->length) break;
        if (al_entry->instance != a->instance) goto do_next_attr;
        /*
         * If the type and/or the name are/is mismatched between the
         * attribute list entry and the attribute record, there is
         * corruption so we break and return error EIO.
         */
        if (al_entry->type != a->type) break;
        if (!ntfs_names_are_equal((ntfschar*) ((char*) a + le16_to_cpu(a->name_offset)), a->name_length, al_name,
                al_name_len, CASE_SENSITIVE, vol->upcase, vol->upcase_len)) break;
        ctx->attr = a;
        /*
         ...
         */
        if ((type == AT_UNUSED) || !val || (!a->non_resident && le32_to_cpu(a->value_length) == val_len && !memcmp(
                (char*) a + le16_to_cpu(a->value_offset), val, val_len)))
        {
            return 0;
        }
        do_next_attr:
 ...
 *	ENOMEM	Not enough memory to allocate necessary buffers.
 */
int ntfs_attr_lookup(const ATTR_TYPES type, const ntfschar *name, const u32 name_len, const IGNORE_CASE_BOOL ic,
        const VCN lowest_vcn, const u8 *val, const u32 val_len, ntfs_attr_search_ctx *ctx)
{
    ...
    if (!ctx || !ctx->mrec || !ctx->attr || (name && name != AT_UNNAMED && (!ctx->ntfs_ino || !(vol
            = ctx->ntfs_ino->vol) || !vol->upcase || !vol->upcase_len)))
    {
        errno = EINVAL;
        ntfs_log_perror("%s", __FUNCTION__);
        goto out;
    ...
    if (!base_ni || !NInoAttrList(base_ni) || type == AT_ATTRIBUTE_LIST)
        ret = ntfs_attr_find(type, name, name_len, ic, val, val_len, ctx);
    else ret = ntfs_external_attr_find(type, name, name_len, ic, lowest_vcn, val, val_len, ctx);
    out:
    ntfs_log_leave("\n");
    return ret;
}
 ...
 */
ATTR_DEF *ntfs_attr_find_in_attrdef(const ntfs_volume *vol, const ATTR_TYPES type)
{
    ATTR_DEF *ad;
    ...
    for (ad = vol->attrdef; (u8*) ad - (u8*) vol->attrdef < vol->attrdef_len && ad->type; ++ad)
    {
        ...
        if (le32_to_cpu(ad->type) < le32_to_cpu(type)) continue;
 ...
 */
int ntfs_attr_size_bounds_check(const ntfs_volume *vol, const ATTR_TYPES type, const s64 size)
{
    ATTR_DEF *ad;
    ...
    if ((min_size && (size < min_size)) || ((max_size > 0) && (size > max_size)))
    {
        errno = ERANGE;
 ...
 *	EINVAL	- Invalid parameters (e.g. @vol is not valid).
 */
static int ntfs_attr_can_be_non_resident(const ntfs_volume *vol, const ATTR_TYPES type, const ntfschar *name,
        int name_len)
{
    ATTR_DEF *ad;
    BOOL allowed;
     ...
     */
    if ((type == AT_LOGGED_UTILITY_STREAM) && name && ntfs_names_are_equal(TXF_DATA, 9, name, name_len, CASE_SENSITIVE,
...
int ntfs_attr_can_be_resident(const ntfs_volume *vol, const ATTR_TYPES type)
{
    ...
    if (type != AT_INDEX_ALLOCATION) return 0;
...
int ntfs_make_room_for_attr(MFT_RECORD *m, u8 *pos, u32 size)
{
    ...
    if (biu + size > le32_to_cpu(m->bytes_allocated) || pos + size > (u8*) m + le32_to_cpu(m->bytes_allocated))
    {
        errno = ENOSPC;
 ...
 */
int ntfs_resident_attr_record_add(ntfs_inode *ni, ATTR_TYPES type, ntfschar *name, u8 name_len, u8 *val, u32 size,
 ...
 *	EIO	- I/O error occurred or damaged filesystem.
 */
int ntfs_non_resident_attr_record_add(ntfs_inode *ni, ATTR_TYPES type, ntfschar *name, u8 name_len, VCN lowest_vcn,
        int dataruns_size, ATTR_FLAGS flags)
{
    ...
    length = offsetof(ATTR_RECORD, compressed_size) + ((sizeof(ntfschar) * name_len + 7) & ~7) + dataruns_size
            + ((flags & (ATTR_IS_COMPRESSED | ATTR_IS_SPARSE)) ? sizeof(a->compressed_size) : 0);
    if (ntfs_make_room_for_attr(ctx->mrec, (u8*) ctx->attr, length))
    {
        err = errno;
    ...
    a->compression_unit = (flags & ATTR_IS_COMPRESSED) ? STANDARD_COMPRESSION_UNIT : 0;
    ...
    if (name_len) memcpy((u8*) a + le16_to_cpu(a->name_offset), name, sizeof(ntfschar) * name_len);
...
int ntfs_attr_record_rm(ntfs_attr_search_ctx *ctx)
{
    ...
    if (ntfs_attr_record_resize(ctx->mrec, ctx->attr, 0))
    {
        ntfs_log_trace("Couldn't remove attribute record. Bug or damaged MFT "
                "record.\n");
        if (NInoAttrList(base_ni) && type != AT_ATTRIBUTE_LIST) if (ntfs_attrlist_entry_add(ni, ctx->attr))
    ...
    {
        if (NInoAttrList(base_ni) && base_ni->attr_list) free(base_ni->attr_list);
 ...
 */
int ntfs_attr_add(ntfs_inode *ni, ATTR_TYPES type, ntfschar *name, u8 name_len, u8 *val, s64 size)
{
    u32 attr_rec_size;
    int err, i, offset;
    BOOL is_resident;
    ...
    if (is_resident)
        attr_rec_size = offsetof(ATTR_RECORD, resident_end) + ((name_len * sizeof(ntfschar) + 7) & ~7) + ((size + 7)
                & ~7);
    else attr_rec_size = offsetof(ATTR_RECORD, non_resident_end) + ((name_len * sizeof(ntfschar) + 7) & ~7) + 8;

    /*
     * If we have enough free space for the new attribute in the base MFT
     * record, then add attribute to it.
     */
    if (le32_to_cpu(ni->mrec->bytes_allocated) - le32_to_cpu(ni->mrec->bytes_in_use) >= attr_rec_size)
    {
        attr_ni = ni;
    ...
    for (i = 0; i < ni->nr_extents; i++)
    {
        ...
        if (le32_to_cpu(attr_ni->mrec->bytes_allocated) - le32_to_cpu(attr_ni->mrec->bytes_in_use) >= attr_rec_size) goto add_attr_record;
    }
    ...
    add_attr_record: if ((ni->flags & FILE_ATTR_COMPRESSED) && (ni->vol->major_ver >= 3) && NVolCompression(ni->vol)
            && (ni->vol->cluster_size <= MAX_COMPRESSION_CLUSTER_SIZE) && ((type == AT_DATA)
            || ((type == AT_INDEX_ROOT) && (name == NTFS_INDEX_I30))))
        data_flags = ATTR_IS_COMPRESSED;
    else data_flags = const_cpu_to_le16(0);
    if (is_resident)
    {
        /* Add resident attribute. */
        offset = ntfs_resident_attr_record_add(attr_ni, type, name, name_len, val, size, data_flags);
        if (offset < 0)
        {
            if (errno == ENOSPC && can_be_non_resident) goto add_non_resident;
            err = errno;
    ...
    offset = ntfs_non_resident_attr_record_add(attr_ni, type, name, name_len, 0, 8, data_flags);
    if (offset < 0)
    {
        err = errno;
    ...
    if (ntfs_attr_truncate(na, size) || (val && (ntfs_attr_pwrite(na, 0, size, val) != size)))
    {
        ...
        if (ntfs_attr_rm(na)) ntfs_log_perror("Failed to remove just added attribute");
        ntfs_attr_close(na);
        goto err_out;
    }
    ntfs_attr_close(na);
    ...
    if (ntfs_attr_record_resize(attr_ni->mrec, (ATTR_RECORD*) ((u8*) attr_ni->mrec + offset), 0)) ntfs_log_perror("Failed to remove just added attribute #2");
    free_err_out:
    /* Free MFT record, if it doesn't contain attributes. */
    if (le32_to_cpu(attr_ni->mrec->bytes_in_use) - le16_to_cpu(attr_ni->mrec->attrs_offset) == 8) if (ntfs_mft_record_free(
            attr_ni->vol, attr_ni)) ntfs_log_perror("Failed to free MFT record");
    err_out: errno = err;
    return -1;
}
...
int ntfs_attr_set_flags(ntfs_inode *ni, ATTR_TYPES type, ntfschar *name, u8 name_len, ATTR_FLAGS flags, ATTR_FLAGS mask)
{
    ntfs_attr_search_ctx *ctx;
    int res;
    ...
    if (ctx)
    {
        if (!ntfs_attr_lookup(type, name, name_len, CASE_SENSITIVE, 0, NULL, 0, ctx))
        {
            /* do the requested change (all small endian le16) */
            ctx->attr->flags = (ctx->attr->flags & ~mask) | (flags & mask);
            NInoSetDirty(ni);
            res = 0;
        }
        ntfs_attr_put_search_ctx(ctx);
...
int ntfs_attr_rm(ntfs_attr *na)
{
    ...
    if (NAttrNonResident(na))
    {
        if (ntfs_attr_map_whole_runlist(na)) return -1;
...
int ntfs_attr_record_resize(MFT_RECORD *m, ATTR_RECORD *a, u32 new_size)
{
    ...
    if (new_size != attr_size)
    {
        ...
        if (a->type == AT_INDEX_ROOT && new_size > attr_size && new_muse + 120 > alloc_size && old_size + 120
                <= alloc_size)
        {
            errno = ENOSPC;
        ...
        memmove((u8 *) a + new_size, (u8 *) a + attr_size, old_size - ((u8 *) a - (u8 *) m) - attr_size);

        /* Adjust @m to reflect the change in used space. */
        m->bytes_in_use = cpu_to_le32(new_muse);

        /* Adjust @a to reflect the new size. */
        if (new_size >= offsetof(ATTR_REC, length) + sizeof(a->length)) a->length = cpu_to_le32(new_size);
    }
    return 0;
}
 ...
 * Note that on error no modifications have been performed whatsoever.
 */
int ntfs_resident_attr_value_resize(MFT_RECORD *m, ATTR_RECORD *a, const u32 new_size)
{
    int ret;
    ...
    if ((ret = ntfs_attr_record_resize(m, a, (le16_to_cpu(a->value_offset) + new_size + 7) & ~7)) < 0) return ret;
    /*
     * If we made the attribute value bigger, clear the area between the
     * old size and @new_size.
     */
    if (new_size > le32_to_cpu(a->value_length)) memset((u8*) a + le16_to_cpu(a->value_offset)
            + le32_to_cpu(a->value_length), 0, new_size - le32_to_cpu(a->value_length));
...
int ntfs_attr_record_move_to(ntfs_attr_search_ctx *ctx, ntfs_inode *ni)
{
    ...
    if (!ntfs_attr_find(a->type, (ntfschar*) ((u8*) a + le16_to_cpu(
            a->name_offset)), a->name_length, CASE_SENSITIVE, NULL, 0, nctx))
    {
    ...
    return 0;
    put_err_out: ntfs_attr_put_search_ctx(nctx);
    errno = err;
    return -1;
}
...
int ntfs_attr_record_move_away(ntfs_attr_search_ctx *ctx, int extra)
{
    ...
    for (i = 0; i < base_ni->nr_extents; i++)
    {
        ...
        if (ctx->ntfs_ino->mft_no == ni->mft_no) continue;

        if (le32_to_cpu(m->bytes_allocated) - le32_to_cpu(m->bytes_in_use) < le32_to_cpu(ctx->attr->length) + extra) continue;
         ...
         * do not return error.
         */
        if (!ntfs_attr_record_move_to(ctx, ni)) return 0;
    }
...
int ntfs_attr_make_non_resident(ntfs_attr *na, ntfs_attr_search_ctx *ctx)
{
    ...
    if (ntfs_attr_can_be_non_resident(vol, na->type, na->name, na->name_len)) return -1;

    new_allocated_size = (le32_to_cpu(a->value_length) + vol->cluster_size - 1) & ~(vol->cluster_size - 1);
    ...
    if (new_allocated_size > 0)
    {
        if ((a->flags & ATTR_COMPRESSION_MASK) == ATTR_IS_COMPRESSED)
        {
            ...
            new_allocated_size = ((new_allocated_size - 1) | ((1L << (STANDARD_COMPRESSION_UNIT
                    + vol->cluster_size_bits)) - 1)) + 1;
        }
        /* Start by allocating clusters to hold the attribute value. */
        rl = ntfs_cluster_alloc(vol, 0, new_allocated_size >> vol->cluster_size_bits, -1, DATA_ZONE);
        if (!rl) return -1;
    }
    else rl = NULL;
    /*
    ...
    if ((a->flags & ATTR_COMPRESSION_MASK) == ATTR_IS_COMPRESSED)
    {
        ...
        na->compression_block_size = 1 << (STANDARD_COMPRESSION_UNIT + vol->cluster_size_bits);
    ...
    if (rl)
    {
        ...
        bw = ntfs_attr_pwrite(na, 0, le32_to_cpu(a->value_length), (u8*) a + le16_to_cpu(a->value_offset));
    ...
    else name_ofs = (sizeof(ATTR_REC) - sizeof(a->compressed_size) + 7) & ~7;
    ...
    if (a->name_length) memmove((u8*) a + name_ofs, (u8*) a + le16_to_cpu(a->name_offset), a->name_length
            * sizeof(ntfschar));
    ...
    if (ntfs_mapping_pairs_build(vol, (u8*) a + mp_ofs, arec_size - mp_ofs, rl, 0, NULL) < 0)
    {
    ...
    cluster_free_err_out: if (rl && ntfs_cluster_free(vol, na, 0, -1) < 0)
 ...
 */
static int ntfs_resident_attr_resize_i(ntfs_attr *na, const s64 newsize, BOOL force_non_resident)
{
    ntfs_attr_search_ctx *ctx;
    ntfs_volume *vol;
    ntfs_inode *ni;
    ...
    if ((newsize < vol->mft_record_size) && !force_non_resident)
    {
        ...
        if (!(ret = ntfs_resident_attr_value_resize(ctx->mrec, ctx->attr, newsize)))
        {
            /* Update attribute size everywhere. */
            na->data_size = na->initialized_size = newsize;
            na->allocated_size = (newsize + 7) & ~7;
            if ((na->data_flags & ATTR_COMPRESSION_MASK) || NAttrSparse(na)) na->compressed_size = na->allocated_size;
            if (na->ni->mrec->flags & MFT_RECORD_IS_DIRECTORY ? na->type == AT_INDEX_ROOT && na->name == NTFS_INDEX_I30
                    : na->type == AT_DATA && na->name == AT_UNNAMED)
            {
                na->ni->data_size = na->data_size;
                if (((na->data_flags & ATTR_COMPRESSION_MASK) || NAttrSparse(na)) && NAttrNonResident(na))
                    na->ni->allocated_size = na->compressed_size;
                else na->ni->allocated_size = na->allocated_size;
                set_nino_flag(na->ni,KnownSize);
                if (na->type == AT_DATA) NInoFileNameSetDirty(na->ni);
            }
            goto resize_done;
        }
    ...
    while (!ntfs_attr_lookup(AT_UNUSED, NULL, 0, 0, 0, NULL, 0, ctx))
    {
        ...
        if (le32_to_cpu(a->length) <= offsetof(ATTR_RECORD,
                compressed_size) + ((a->name_length * sizeof(ntfschar) + 7) & ~7) + 8) continue;
        ...
        if (((tna->data_flags & ATTR_COMPRESSION_MASK) == ATTR_IS_COMPRESSED) && ntfs_attr_pclose(tna))
        {
            err = errno;
            ntfs_attr_close(tna);
            goto put_err_out;
    ...
    if (ntfs_attr_lookup(na->type, na->name, na->name_len, CASE_SENSITIVE, 0, NULL, 0, ctx))
    {
     ...
     */
    if (le32_to_cpu(ctx->mrec->bytes_in_use) == le16_to_cpu(ctx->mrec->attrs_offset) + le32_to_cpu(ctx->attr->length)
            + 8)
    {
        err = ENOSPC;
    ...
    {
        ntfs_attr_put_search_ctx(ctx);
        if (ntfs_inode_add_attrlist(ni)) return -1;
        return ntfs_resident_attr_resize_i(na, newsize, force_non_resident);
    }
    ...
    if (na->ni->nr_extents == -1) na->ni = ni;
...
static int ntfs_attr_make_resident(ntfs_attr *na, ntfs_attr_search_ctx *ctx)
{
    ...
    if (ntfs_attr_can_be_resident(vol, na->type)) return -1;
    ...
    if (le32_to_cpu(ctx->mrec->bytes_in_use) - le32_to_cpu(a->length) + arec_size
    ...
    if (a->name_length)
    {
        memmove((u8*) a + name_ofs, (u8*) a + le16_to_cpu(a->name_offset), a->name_length * sizeof(ntfschar));
    }
    a->name_offset = cpu_to_le16(name_ofs);
     ...
     */
    if (!na->data_size && (na->type == AT_DATA) && (na->ni->vol->major_ver >= 3) && NVolCompression(na->ni->vol)
            && (na->ni->vol->cluster_size <= MAX_COMPRESSION_CLUSTER_SIZE) && (na->ni->flags & FILE_ATTR_COMPRESSED))
    {
    ...
    if (a->type == AT_FILE_NAME)
        a->resident_flags = RESIDENT_ATTR_IS_INDEXED;
    else a->resident_flags = 0;
    a->reservedR = 0;

    /* Sanity fixup...  Shouldn't really happen. (AIA) */
    if (na->initialized_size > na->data_size) na->initialized_size = na->data_size;

    /* Copy data from run list to resident attribute value. */
    bytes_read = ntfs_rl_pread(vol, na->rl, 0, na->initialized_size, (u8*) a + val_ofs);
    if (bytes_read != na->initialized_size)
    {
        if (bytes_read < 0) err = errno;
    ...
    if (na->initialized_size < na->data_size) memset((u8*) a + val_ofs + na->initialized_size, 0, na->data_size
            - na->initialized_size);
 ...
 */
static int ntfs_attr_update_meta(ATTR_RECORD *a, ntfs_attr *na, MFT_RECORD *m, ntfs_attr_search_ctx *ctx)
{
    int sparse, ret = 0;
    ...
    if (sparse && !(a->flags & (ATTR_IS_SPARSE | ATTR_IS_COMPRESSED)))
    {
         ...
         */
        if ((le32_to_cpu(a->length) - le16_to_cpu(a->mapping_pairs_offset) == 8) && !(le32_to_cpu(m->bytes_allocated)
                - le32_to_cpu(m->bytes_in_use)))
        {
            ...
            {
                ntfs_attr_put_search_ctx(ctx);
                if (ntfs_inode_add_attrlist(na->ni)) goto leave;
                goto retry;
            }
        ...
        memmove((u8*) a + le16_to_cpu(a->name_offset) + 8, (u8*) a + le16_to_cpu(a->name_offset), a->name_length
                * sizeof(ntfschar));

        a->name_offset = cpu_to_le16(le16_to_cpu(a->name_offset) + 8);

        a->mapping_pairs_offset = cpu_to_le16(le16_to_cpu(a->mapping_pairs_offset) + 8);
    }

    /* Attribute no longer sparse. */
    if (!sparse && (a->flags & ATTR_IS_SPARSE) && !(a->flags & ATTR_IS_COMPRESSED))
    {
        ...
        if (le16_to_cpu(a->name_offset) >= 8) a->name_offset = cpu_to_le16(le16_to_cpu(a->name_offset) - 8);
    ...
    if (sparse || (na->data_flags & ATTR_COMPRESSION_MASK))
    {
        ...
        if (new_compr_size == -1) goto error;
...
static int ntfs_attr_update_mapping_pairs_i(ntfs_attr *na, VCN from_vcn)
{
    ...
    while (!ntfs_attr_lookup(na->type, na->name, na->name_len, CASE_SENSITIVE, from_vcn, NULL, 0, ctx))
    {
        a = ctx->attr;
        m = ctx->mrec;
        if (!a->lowest_vcn) first_updated = TRUE;
        /*
        ...
        if (from_vcn)
        {
            ...
            if (first_lcn == LCN_ENOENT || first_lcn == LCN_RL_NOT_MAPPED) finished_build = TRUE;
        }
         ...
         */
        cur_max_mp_size = le32_to_cpu(a->length) - le16_to_cpu(a->mapping_pairs_offset);
        /*
         * Determine maximum possible length of mapping pairs in the
         * current mft record, if we shall expand space for mapping
         * pairs.
         */
        exp_max_mp_size = le32_to_cpu(m->bytes_allocated) - le32_to_cpu(m->bytes_in_use) + cur_max_mp_size;
        /* Get the size for the rest of mapping pairs array. */
        mp_size = ntfs_get_size_for_mapping_pairs(na->ni->vol, stop_rl, stop_vcn, exp_max_mp_size);
        if (mp_size <= 0)
        {
        ...
        if (mp_size > exp_max_mp_size)
        {
            ...
            {
                ntfs_attr_put_search_ctx(ctx);
                if (ntfs_inode_free_space(na->ni, mp_size - cur_max_mp_size))
                {
        ...
        {
            if (ntfs_attr_record_resize(m, a, le16_to_cpu(a->mapping_pairs_offset) + mp_size))
            {
                errno = EIO;
        ...
        ntfs_inode_mark_dirty(ctx->ntfs_ino);
        if ((ctx->ntfs_ino->nr_extents == -1 || NInoAttrList(ctx->ntfs_ino)) && ctx->attr->type != AT_ATTRIBUTE_LIST)
        {
        ...
        if (!ntfs_mapping_pairs_build(na->ni->vol, (u8*) a + le16_to_cpu(
                a->mapping_pairs_offset), mp_size, na->rl, stop_vcn, &stop_rl)) finished_build = TRUE;
        if (stop_rl)
    ...
    if (!first_updated)
    {
        ...
        if (!ntfs_attr_lookup(na->type, na->name, na->name_len, CASE_SENSITIVE, 0, NULL, 0, ctx))
        {
            a = ctx->attr;
            a->allocated_size = cpu_to_sle64(na->allocated_size);
            spcomp = na->data_flags & (ATTR_IS_COMPRESSED | ATTR_IS_SPARSE);
            if (spcomp) a->compressed_size = cpu_to_sle64(na->compressed_size);
            if ((na->type == AT_DATA) && (na->name == AT_UNNAMED))
            {
                na->ni->allocated_size = (spcomp ? na->compressed_size : na->allocated_size);
                NInoFileNameSetDirty(na->ni);
            }
        }
        else
    ...
    if (finished_build)
    {
        ...
        while (!ntfs_attr_lookup(na->type, na->name, na->name_len, CASE_SENSITIVE, 0, NULL, 0, ctx))
        {
            if (sle64_to_cpu(ctx->attr->highest_vcn) != NTFS_VCN_DELETE_MARK) continue;
    ...
    while (1)
    {
        ...
        mp_size = ntfs_get_size_for_mapping_pairs(na->ni->vol, na->rl, stop_vcn, INT_MAX);
        if (mp_size <= 0)
        {
            ntfs_log_perror("%s: get mp size failed", __FUNCTION__);
            goto put_err_out;
         ...
         * possible maximum.
         */
        cur_max_mp_size = le32_to_cpu(m->bytes_allocated) - le32_to_cpu(m->bytes_in_use)
                - (offsetof(ATTR_RECORD, compressed_size) + (((na->data_flags & ATTR_COMPRESSION_MASK) || NAttrSparse(
                        na)) ? sizeof(a->compressed_size) : 0)) - ((sizeof(ntfschar) * na->name_len + 7) & ~7);
        if (mp_size > cur_max_mp_size) mp_size = cur_max_mp_size;
        /* Add attribute extent to new record. */
        err
                = ntfs_non_resident_attr_record_add(ni, na->type, na->name, na->name_len, stop_vcn, mp_size,
                        na->data_flags);
        if (err == -1)
        {
            err = errno;
            ntfs_log_perror("Could not add attribute extent");
            if (ntfs_mft_record_free(na->ni->vol, ni)) ntfs_log_perror("Could not free MFT record");
            errno = err;
            goto put_err_out;
        }
        a = (ATTR_RECORD*) ((u8*) m + err);

        err = ntfs_mapping_pairs_build(na->ni->vol, (u8*) a + le16_to_cpu(a->mapping_pairs_offset), mp_size, na->rl,
                stop_vcn, &stop_rl);
        if (stop_rl)
            stop_vcn = stop_rl->vcn;
        else stop_vcn = 0;
        if (err < 0 && errno != ENOSPC)
        {
            ...
            if (ntfs_mft_record_free(na->ni->vol, ni)) ntfs_log_perror("Couldn't free MFT record");
            errno = err;
            goto put_err_out;
        }
        a->highest_vcn = cpu_to_sle64(stop_vcn - 1);
    ...
    ok: ret = 0;
    out: return ret;
    put_err_out: if (ctx) ntfs_attr_put_search_ctx(ctx);
    goto out;
}
...
static int ntfs_non_resident_attr_shrink(ntfs_attr *na, const s64 newsize)
{
    ...
    if (na->data_flags & ATTR_COMPRESSION_MASK)
         ...
         */
        first_free_vcn = (((newsize - 1) | (na->compression_block_size - 1)) + 1) >> vol->cluster_size_bits;
    else first_free_vcn = (newsize + vol->cluster_size - 1) >> vol->cluster_size_bits;
    /*
    ...
    if (!newsize)
    {
        if (ntfs_attr_make_resident(na, ctx))
        {
            ...
            if (errno != EPERM) ntfs_log_error("Failed to make attribute "
...
static int ntfs_non_resident_attr_expand_i(ntfs_attr *na, const s64 newsize)
{
    ...
    /* The first cluster outside the new allocation. */
    first_free_vcn = (newsize + vol->cluster_size - 1) >> vol->cluster_size_bits;
    /*
    ...
    if ((na->allocated_size >> vol->cluster_size_bits) < first_free_vcn)
    {
        ...
        if (na->type == AT_DATA && vol->major_ver >= 3)
        {
            ...
            rl[0].vcn = (na->allocated_size >> vol->cluster_size_bits);
            rl[0].lcn = LCN_HOLE;
            rl[0].length = first_free_vcn - (na->allocated_size >> vol->cluster_size_bits);
        ...
        else
        {
            ...
            if (na->rl->length)
            {
                 ...
                 */
                if (rl->lcn >= 0) lcn_seek_from = rl->lcn + rl->length;
            }

            rl = ntfs_cluster_alloc(vol, na->allocated_size >> vol->cluster_size_bits, first_free_vcn
                    - (na->allocated_size >> vol->cluster_size_bits), lcn_seek_from, DATA_ZONE);
            if (!rl)
            {
    ...
    if (ntfs_cluster_free(vol, na, org_alloc_size >> vol->cluster_size_bits, -1) < 0)
    {
        err = EIO;
        ntfs_log_perror("Leaking clusters");
    }
    /* Now, truncate the runlist itself. */
    if (ntfs_rl_truncate(&na->rl, org_alloc_size >> vol->cluster_size_bits))
    {
        /*
...
int ntfs_attr_truncate(ntfs_attr *na, const s64 newsize)
{
     ...
     */
    compressed = (na->data_flags & ATTR_COMPRESSION_MASK) != const_cpu_to_le16(0);
    if (compressed && NAttrNonResident(na) && ((na->data_flags & ATTR_COMPRESSION_MASK) != ATTR_IS_COMPRESSED))
    {
        errno = EOPNOTSUPP;
    ...
    if (NAttrNonResident(na))
    {
        ...
        if (compressed && newsize && (newsize > na->data_size))
            fullsize = (na->initialized_size | (na->compression_block_size - 1)) + 1;
        else fullsize = newsize;
        if (fullsize > na->data_size)
            ret = ntfs_non_resident_attr_expand(na, fullsize);
        else ret = ntfs_non_resident_attr_shrink(na, fullsize);
    }
    else ret = ntfs_resident_attr_resize(na, newsize);
    out:
...
static int stuff_hole(ntfs_attr *na, const s64 pos)
{
    ...
    if (!NAttrNonResident(na))
    {
        ...
        if (!ret && !NAttrNonResident(na)) na->initialized_size = na->data_size = pos;
    }
    if (!ret && NAttrNonResident(na))
    {
        /* does the hole span over several compression block ? */
        if ((pos ^ na->initialized_size) & ~(na->compression_block_size - 1))
        {
            begin_size = ((na->initialized_size - 1) | (na->compression_block_size - 1)) + 1 - na->initialized_size;
        ...
        if (buf || !size)
        {
            ...
            if (begin_size && (ntfs_attr_pwrite(na, na->initialized_size, begin_size, buf) != begin_size)) ret = -1;
            /* create an unstuffed hole */
            if (!ret && ((na->initialized_size + end_size) < pos) && ntfs_non_resident_attr_expand(na, pos - end_size))
                ret = -1;
            else na->initialized_size = na->data_size = pos - end_size;
            /* stuff into the target block */
            if (!ret && end_size && (ntfs_attr_pwrite(na, na->initialized_size, end_size, buf) != end_size)) ret = -1;
            if (buf) free(buf);
        }
        else ret = -1;
    }
 ...
 */
void *ntfs_attr_readall(ntfs_inode *ni, const ATTR_TYPES type, ntfschar *name, u32 name_len, s64 *data_size)
{
    ntfs_attr *na;
    void *data, *ret = NULL;
    s64 size;
    ...
    out: ntfs_attr_close(na);
    err_exit:
    ntfs_log_leave("\n");
    return ret;
}

int ntfs_attr_exist(ntfs_inode *ni, const ATTR_TYPES type, ntfschar *name, u32 name_len)
{
    ntfs_attr_search_ctx *ctx;
    int ret;
    ...
    ret = ntfs_attr_lookup(type, name, name_len, CASE_SENSITIVE, 0, NULL, 0, ctx);
...
int ntfs_attr_remove(ntfs_inode *ni, const ATTR_TYPES type, ntfschar *name, u32 name_len)
{
    ...
    ret = ntfs_attr_rm(na);
    if (ret) ntfs_log_perror("Failed to remove attribute 0x%02x of inode "
            "0x%llx", type, (unsigned long long)ni->mft_no);
    ntfs_attr_close(na);
...
s64 ntfs_attr_get_free_bits(ntfs_attr *na)
{
    ...
    while (1)
    {
        ...
        for (; (u8 *) p >= buf; p--)
        {
            nr_free += lut[*p & 255] + lut[(*p >> 8) & 255] + lut[(*p >> 16) & 255] + lut[(*p >> 24)];
        }
        switch (br % 4)
        {
            case 3:
