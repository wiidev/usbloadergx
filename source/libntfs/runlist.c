...
{
    if (size > 0) memcpy(dstbase + dst, srcbase + src, size * sizeof(*dstbase));
}
 ...
 */
static runlist_element *ntfs_rl_realloc(runlist_element *rl, int old_size, int new_size)
{
...
runlist_element *ntfs_rl_extend(ntfs_attr *na, runlist_element *rl, int more_entries)
{
...
static BOOL ntfs_rl_are_mergeable(runlist_element *dst, runlist_element *src)
{
    ...
    if ((dst->lcn == LCN_RL_NOT_MAPPED) && (src->lcn == LCN_RL_NOT_MAPPED)) return TRUE;
    /* If the runs are misaligned, we cannot merge them. */
    if ((dst->vcn + dst->length) != src->vcn) return FALSE;
    /* If both runs are non-sparse and contiguous, we can merge them. */
    if ((dst->lcn >= 0) && (src->lcn >= 0) && ((dst->lcn + dst->length) == src->lcn)) return TRUE;
    /* If we are merging two holes, we can merge them. */
    if ((dst->lcn == LCN_HOLE) && (src->lcn == LCN_HOLE)) return TRUE;
    /* Cannot merge. */
    return FALSE;
}
 ...
 * left unmodified.
 */
static runlist_element *ntfs_rl_append(runlist_element *dst, int dsize, runlist_element *src, int ssize, int loc)
{
    ...
    if ((loc + 1) < dsize) right = ntfs_rl_are_mergeable(src + ssize - 1, dst + loc + 1);
    ...
    if (right) __ntfs_rl_merge(src + ssize - 1, dst + loc + 1);
    ...
    if (dst[marker].lcn == LCN_ENOENT) dst[marker].vcn = dst[marker - 1].vcn + dst[marker - 1].length;
 ...
 * left unmodified.
 */
static runlist_element *ntfs_rl_insert(runlist_element *dst, int dsize, runlist_element *src, int ssize, int loc)
{
    ...
    else
    {
        ...
        if (left) merged_length += src->length;
    ...
    if (left) __ntfs_rl_merge(dst + loc - 1, src);
    ...
    /* ... and the length. */
    if (dst[marker].lcn == LCN_HOLE || dst[marker].lcn == LCN_RL_NOT_MAPPED) dst[marker].length = dst[marker + 1].vcn
            - dst[marker].vcn;
 ...
 * left unmodified.
 */
static runlist_element *ntfs_rl_replace(runlist_element *dst, int dsize, runlist_element *src, int ssize, int loc)
{
    signed delta;
    BOOL left = FALSE; /* Left end of @src needs merging */
    BOOL right = FALSE; /* Right end of @src needs merging */
    ...
    if ((loc + 1) < dsize) right = ntfs_rl_are_mergeable(src + ssize - 1, dst + loc + 1);
    if (loc > 0) left = ntfs_rl_are_mergeable(dst + loc - 1, src);
    ...
    /* We may have changed the length of the file, so fix the end marker */
    if (((dsize - tail) > 0) && (dst[marker].lcn == LCN_ENOENT)) dst[marker].vcn = dst[marker - 1].vcn
            + dst[marker - 1].length;
 ...
 * left unmodified.
 */
static runlist_element *ntfs_rl_split(runlist_element *dst, int dsize, runlist_element *src, int ssize, int loc)
{
    if (!dst || !src)
    {
 ...
 */
static runlist_element *ntfs_runlists_merge_i(runlist_element *drl, runlist_element *srl)
{
    ...
    if ((drl[di].vcn == srl[si].vcn) && (drl[di].lcn >= 0) && (srl[si].lcn >= 0))
    {
        errno = ERANGE;
    ...
    if (srl[send].lcn == (LCN) LCN_ENOENT) marker_vcn = srl[marker = send].vcn;
...

    {
        ...
        if (start)
        {
            ...
            else drl = ntfs_rl_insert(drl, ds, srl + sstart, ss, dins);
        }
        else
        {
            if (finish)
                drl = ntfs_rl_append(drl, ds, srl + sstart, ss, dins);
            else drl = ntfs_rl_split(drl, ds, srl + sstart, ss, dins);
        }
        if (!drl)
        {
            ...
            if (drl[ds].vcn <= marker_vcn)
            {
                ...
                if (drl[ds].lcn != (LCN) LCN_RL_NOT_MAPPED)
                {
                    ...
                    if (!slots)
                    {
                        ...
                        if (!drl) goto critical_error;
                        slots = 2;
                    }
                    ds++;
                    /* Need to set vcn if it isn't set already. */
                    if (slots != 1) drl[ds].vcn = drl[ds - 1].vcn + drl[ds - 1].length;
 ...
 */
static runlist_element *ntfs_mapping_pairs_decompress_i(const ntfs_volume *vol, const ATTR_RECORD *attr,
        runlist_element *old_rl)
{
    ...
    if (!attr || !attr->non_resident || sle64_to_cpu(attr->lowest_vcn) < (VCN) 0)
    {
        errno = EINVAL;
        return NULL;
    }
    ...
    if (!attr->lowest_vcn)
    {
        ...
        max_cluster = ((sle64_to_cpu(attr->allocated_size) + vol->cluster_size - 1) >> vol->cluster_size_bits) - 1;
        /*
...
LCN ntfs_rl_vcn_to_lcn(const runlist_element *rl, const VCN vcn)
{
    ...
    for (i = 0; rl[i].length; i++)
    {
        if (vcn < rl[i + 1].vcn)
        {
            if (rl[i].lcn >= (LCN) 0) return rl[i].lcn + (vcn - rl[i].vcn);
            return rl[i].lcn;
        }
    }
    /*
 ...
 */
s64 ntfs_rl_pread(const ntfs_volume *vol, const runlist_element *rl, const s64 pos, s64 count, void *b)
{
    ...
    for (ofs = 0; rl->length && (ofs + (rl->length << vol->cluster_size_bits) <= pos); rl++)
    ...
    for (total = 0LL; count; rl++, ofs = 0)
    {
        ...
        to_read = min(count, (rl->length << vol->cluster_size_bits) -
                ofs);
        retry: bytes_read = ntfs_pread(vol->dev, (rl->lcn << vol->cluster_size_bits) + ofs, to_read, b);
        ...
        if (bytes_read == (s64) -1 && errno == EINTR) goto retry;
        if (bytes_read == (s64) -1) err = errno;
        goto rl_err_out;
    }
...
s64 ntfs_rl_pwrite(const ntfs_volume *vol, const runlist_element *rl, s64 ofs, const s64 pos, s64 count, void *b)
{
    ...
    for (total = 0LL; count; rl++, ofs = 0)
    {
        ...
        if (written == (s64) -1) err = errno;
        goto rl_err_out;
    }
    out: return total;
    rl_err_out: if (total) goto out;
    errno = err;
    errno_set: total = -1;
    goto out;
}
 ...
 */
int ntfs_get_size_for_mapping_pairs(const ntfs_volume *vol, const runlist_element *rl, const VCN start_vcn,
        int max_size)
{
    LCN prev_lcn;
    int rls;
    ...
    out: return rls;
    err_out: if (rl->lcn == LCN_RL_NOT_MAPPED)
        errno = EINVAL;
    else errno = EIO;
    errno_set: rls = -1;
    goto out;
 ...
 */
int ntfs_mapping_pairs_build(const ntfs_volume *vol, u8 *dst, const int dst_len, const runlist_element *rl,
        const VCN start_vcn, runlist_element const **stop_rl)
{
    ...
    if (start_vcn < 0) goto val_err;
    if (!rl)
    {
        if (start_vcn) goto val_err;
        if (stop_rl) *stop_rl = rl;
        if (dst_len < 1) goto nospc_err;
        goto ok;
    }
    /* Skip to runlist element containing @start_vcn. */
    while (rl->length && start_vcn >= rl[1].vcn)
        rl++;
    if ((!rl->length && start_vcn > rl->vcn) || start_vcn < rl->vcn) goto val_err;
    /*
    ...
    if (start_vcn > rl->vcn)
    {
        ...
        delta = start_vcn - rl->vcn;
        /* Write length. */
        len_len = ntfs_write_significant_bytes(dst + 1, dst_max, rl->length - delta);
        if (len_len < 0) goto size_err;
        /*
         * If the logical cluster number (lcn) denotes a hole and we
         * are on NTFS 3.0+, we don't store it at all, i.e. we need
        ...
        if (rl->lcn >= 0 || vol->major_ver < 3)
        {
            ...
            if (rl->lcn >= 0) prev_lcn += delta;
            /* Write change in lcn. */
            lcn_len = ntfs_write_significant_bytes(dst + 1 + len_len, dst_max, prev_lcn);
            if (lcn_len < 0) goto size_err;
        }
        else lcn_len = 0;
        dst_next = dst + len_len + lcn_len + 1;
        if (dst_next > dst_max) goto size_err;
        /* Update header byte. */
    ...
    for (; rl->length; rl++)
    {
        if (rl->length < 0 || rl->lcn < LCN_HOLE) goto err_out;
        /* Write length. */
        len_len = ntfs_write_significant_bytes(dst + 1, dst_max, rl->length);
        if (len_len < 0) goto size_err;
        /*
         * If the logical cluster number (lcn) denotes a hole and we
         * are on NTFS 3.0+, we don't store it at all, i.e. we need
        ...
        {
            /* Write change in lcn. */
            lcn_len = ntfs_write_significant_bytes(dst + 1 + len_len, dst_max, rl->lcn - prev_lcn);
            if (lcn_len < 0) goto size_err;
            prev_lcn = rl->lcn;
        }
        else lcn_len = 0;
    ...
    *dst = 0;
    ...
    /* Add terminator byte. */
    *dst = 0;
    nospc_err: errno = ENOSPC;
    goto errno_set;
    val_err: errno = EINVAL;
    goto errno_set;
    err_out: if (rl->lcn == LCN_RL_NOT_MAPPED)
        errno = EINVAL;
    else errno = EIO;
