...
const devoptab_t *ntfsGetDevice(const char *path, bool useDefaultDevice)
{
    ...
    if (useDefaultDevice) return GetDeviceOpTab("");
...
ntfs_vd *ntfsGetVolume(const char *path)
{
    ...
    if (devoptab && devoptab_ntfs && (devoptab->open_r == devoptab_ntfs->open_r)) return (ntfs_vd*) devoptab->deviceData;
...
ntfs_inode *ntfsParseEntry(ntfs_vd *vd, const char *path, int reparseLevel)
{
    ...
    else ni = ntfs_pathname_to_inode(vd->vol, NULL, path);
...
void ntfsCloseEntry(ntfs_vd *vd, ntfs_inode *ni)
{
    ...
    if (NInoDirty(ni)) ntfsSync(vd, ni);
...
ntfs_inode *ntfsCreate(ntfs_vd *vd, const char *path, mode_t type, const char *target)
{
    ...
    if (dir_ni) ntfsCloseEntry(vd, dir_ni);

    if (utarget) ntfs_free(utarget);

    if (uname) ntfs_free(uname);
