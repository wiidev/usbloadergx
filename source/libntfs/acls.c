...
static const char nullsidbytes[] = { 1, /* revision */
1, /* auth count */
0, 0, 0, 0, 0, 0, /* base */
0, 0, 0, 0 /* 1st level */
};
...
static const char worldsidbytes[] = { 1, /* revision */
1, /* auth count */
0, 0, 0, 0, 0, 1, /* base */
0, 0, 0, 0 /* 1st level */
};
...
static const char adminsidbytes[] = { 1, /* revision */
2, /* auth count */
0, 0, 0, 0, 0, 5, /* base */
32, 0, 0, 0, /* 1st level */
32, 2, 0, 0 /* 2nd level */
...
static const char systemsidbytes[] = { 1, /* revision */
1, /* auth count */
0, 0, 0, 0, 0, 5, /* base */
18, 0, 0, 0 /* 1st level */
};
...
static const char ownersidbytes[] = { 1, /* revision */
1, /* auth count */
0, 0, 0, 0, 0, 3, /* base */
0, 0, 0, 0 /* 1st level */
};
...
static const char groupsidbytes[] = { 1, /* revision */
1, /* auth count */
0, 0, 0, 0, 0, 3, /* base */
1, 0, 0, 0 /* 1st level */
};
...
BOOL ntfs_same_sid(const SID *first, const SID *second)
{
    ...
    return ((ntfs_sid_size(second) == size) && !memcmp(first, second, size));
}
...
static int is_world_sid(const SID * usid)
{
    ...
    ((usid->sub_authority_count == 1) && (usid->identifier_authority.high_part == const_cpu_to_be16(0))
            && (usid->identifier_authority.low_part == const_cpu_to_be32(1)) && (usid->sub_authority[0]
            == const_cpu_to_le32(0)))

    /* check whether S-1-5-32-545 : local user */
    || ((usid->sub_authority_count == 2) && (usid->identifier_authority.high_part == const_cpu_to_be16(0))
            && (usid->identifier_authority.low_part == const_cpu_to_be32(5)) && (usid->sub_authority[0]
            == const_cpu_to_le32(32)) && (usid->sub_authority[1] == const_cpu_to_le32(545))));
}
...
{
    return ((usid->sub_authority_count == 5) && (usid->identifier_authority.high_part == const_cpu_to_be16(0))
            && (usid->identifier_authority.low_part == const_cpu_to_be32(5)) && (usid->sub_authority[0]
...
unsigned int ntfs_attr_size(const char *attr)
{
    ...
    if (offsacl >= attrsz)
    {
        ...
        if (endacl > attrsz) attrsz = endacl;
    }
...
{
    return ((sid->revision == SID_REVISION) && (sid->sub_authority_count >= 1) && (sid->sub_authority_count <= 8));
}
    ...
    if (xsid->sub_authority_count == cnt)
    {
        ...
        if ((xlast > rlast) && !((xlast ^ rlast ^ parity) & 1))
        {
            ...
            else
            {
                ...
                if (carry < 4) xid = (((xlast - rlast) >> 1) & 0x3fffffff) | (carry << 30);
            }
        }
    }
    return (xid);
...
const SID *ntfs_find_usid(const struct MAPPING* usermapping, uid_t uid, SID *defusid)
{
    ...
    else
    {
        ...
        else sid = (p ? p->sid : (const SID*) NULL);
    }
    return (sid);
}
...
const SID *ntfs_find_gsid(const struct MAPPING* groupmapping, gid_t gid, SID *defgsid)
{
    const struct MAPPING *p;
    const SID *sid;
    le32 leauth;
...
static BOOL valid_acl(const ACL *pacl, unsigned int end)
{
    ...
    for (nace = 0; (nace < acecnt) && ok; nace++)
    {
        ...
        {
            pace = (const ACCESS_ALLOWED_ACE*) &((const char*) pacl)[offace];
            acesz = le16_to_cpu(pace->size);
            if (((offace + acesz) > end) || !ntfs_valid_sid(&pace->sid) || ((ntfs_sid_size(&pace->sid) + 8)
                    != (int) acesz)) ok = FALSE;
            offace += acesz;
        }
    }
...
BOOL ntfs_valid_descr(const char *securattr, unsigned int attrsz)
{
     ...
     */
    if ((attrsz >= sizeof(SECURITY_DESCRIPTOR_RELATIVE)) && (phead->revision == SECURITY_DESCRIPTOR_REVISION)
            && (offowner >= sizeof(SECURITY_DESCRIPTOR_RELATIVE)) && ((offowner + 2) < attrsz) && (offgroup
            >= sizeof(SECURITY_DESCRIPTOR_RELATIVE)) && ((offgroup + 2) < attrsz) && (!offdacl || ((offdacl
            >= sizeof(SECURITY_DESCRIPTOR_RELATIVE)) && (offdacl + sizeof(ACL) < attrsz))) && (!offsacl || ((offsacl
            >= sizeof(SECURITY_DESCRIPTOR_RELATIVE)) && (offsacl + sizeof(ACL) < attrsz))) && !(phead->owner
            & const_cpu_to_le32(3)) && !(phead->group & const_cpu_to_le32(3)) && !(phead->dacl & const_cpu_to_le32(3))
            && !(phead->sacl & const_cpu_to_le32(3)) && (ntfs_attr_size(securattr) <= attrsz) && ntfs_valid_sid(
            (const SID*) &securattr[offowner]) && ntfs_valid_sid((const SID*) &securattr[offgroup])
    /*
     * if there is an ACL, as indicated by offdacl,
     * require SE_DACL_PRESENT
     * but "Dr Watson" has SE_DACL_PRESENT though no DACL
     */
    && (!offdacl || ((phead->control & SE_DACL_PRESENT) && ((pdacl->revision == ACL_REVISION) || (pdacl->revision
            == ACL_REVISION_DS))))
    /* same for SACL */
    && (!offsacl || ((phead->control & SE_SACL_PRESENT) && ((psacl->revision == ACL_REVISION) || (psacl->revision
            == ACL_REVISION_DS)))))
    {
        /*
         *  Check the DACL and SACL if present
         */
        if ((offdacl && !valid_acl(pdacl, attrsz - offdacl)) || (offsacl && !valid_acl(psacl, attrsz - offsacl))) ok
                = FALSE;
    }
    else ok = FALSE;
    return (ok);
...
int ntfs_inherit_acl(const ACL *oldacl, ACL *newacl, const SID *usid, const SID *gsid, BOOL fordir)
{
    ...
    for (nace = 0; nace < oldcnt; nace++)
    {
        ...
        {
            pnewace = (ACCESS_ALLOWED_ACE*) ((char*) newacl + dst);
            ...
            if (pnewace->mask & GENERIC_ALL)
            {
                ...
                if (fordir)
                    pnewace->mask |= OWNER_RIGHTS | DIR_READ | DIR_WRITE | DIR_EXEC;
                else
                /*
                 * The last flag is not defined for a file,
                 * however Windows sets it, so do the same
                 */
                pnewace->mask |= OWNER_RIGHTS | FILE_READ | FILE_WRITE | FILE_EXEC | cpu_to_le32(0x40);
            }
            /* remove inheritance flags */
            pnewace->flags &= ~(OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE | INHERIT_ONLY_ACE);
            dst += acesz;
            newcnt++;
        }
        /* inheritance for further inheritance */
        if (fordir && (poldace->flags & (CONTAINER_INHERIT_ACE | OBJECT_INHERIT_ACE)))
        {
            pnewace = (ACCESS_ALLOWED_ACE*) ((char*) newacl + dst);
            memcpy(pnewace, poldace, acesz);
            /*
...
static int buildacls(char *secattr, int offs, mode_t mode, int isdir, const SID * usid, const SID * gsid)
{
    ACL *pacl;
    ACCESS_ALLOWED_ACE *pgace;
    ACCESS_ALLOWED_ACE *pdace;
    ...
    asidsz = ntfs_sid_size(adminsid);
    ssidsz = ntfs_sid_size(systemsid);
    adminowns = ntfs_same_sid(usid, adminsid) || ntfs_same_sid(gsid, adminsid);
    groupowns = !adminowns && ntfs_same_sid(usid, gsid);
    ...
    if (isdir)
    {
        ...
        if (mode & S_IXUSR) grants |= DIR_EXEC;
        if (mode & S_IWUSR) grants |= DIR_WRITE;
        if (mode & S_IRUSR) grants |= DIR_READ;
    }
    else
    {
        gflags = FILE_INHERITANCE;
        if (mode & S_IXUSR) grants |= FILE_EXEC;
        if (mode & S_IWUSR) grants |= FILE_WRITE;
        if (mode & S_IRUSR) grants |= FILE_READ;
    }
    ...
    if (!adminowns)
    {
        if (!groupowns)
        {
            if (isdir)
            {
                ...
                if (mode & (S_IXGRP | S_IXOTH)) denials |= DIR_EXEC;
                if (mode & (S_IWGRP | S_IWOTH)) denials |= DIR_WRITE;
                if (mode & (S_IRGRP | S_IROTH)) denials |= DIR_READ;
            }
            else
            {
                pdace->flags = FILE_INHERITANCE;
                if (mode & (S_IXGRP | S_IXOTH)) denials |= FILE_EXEC;
                if (mode & (S_IWGRP | S_IWOTH)) denials |= FILE_WRITE;
                if (mode & (S_IRGRP | S_IROTH)) denials |= FILE_READ;
            }
        }
        else
        {
            ...
            {
                pdace->flags = DIR_INHERITANCE;
                if ((mode & S_IXOTH) && !(mode & S_IXGRP)) denials |= DIR_EXEC;
                if ((mode & S_IWOTH) && !(mode & S_IWGRP)) denials |= DIR_WRITE;
                if ((mode & S_IROTH) && !(mode & S_IRGRP)) denials |= DIR_READ;
            }
            else
            {
                pdace->flags = FILE_INHERITANCE;
                if ((mode & S_IXOTH) && !(mode & S_IXGRP)) denials |= FILE_EXEC;
                if ((mode & S_IWOTH) && !(mode & S_IWGRP)) denials |= FILE_WRITE;
                if ((mode & S_IROTH) && !(mode & S_IRGRP)) denials |= FILE_READ;
            }
        }
        denials &= ~grants;
        if (denials)
    ...
    if (adminowns || groupowns || (((mode >> 3) ^ mode) & 7))
    {
        grants = WORLD_RIGHTS;
        if (isdir)
        {
            gflags = DIR_INHERITANCE;
            if (mode & S_IXGRP) grants |= DIR_EXEC;
            if (mode & S_IWGRP) grants |= DIR_WRITE;
            if (mode & S_IRGRP) grants |= DIR_READ;
        }
        else
        {
            gflags = FILE_INHERITANCE;
            if (mode & S_IXGRP) grants |= FILE_EXEC;
            if (mode & S_IWGRP) grants |= FILE_WRITE;
            if (mode & S_IRGRP) grants |= FILE_READ;
        }
        ...
        if (!adminowns && !groupowns)
        {
            ...
            {
                pdace->flags = DIR_INHERITANCE;
                if (mode & S_IXOTH) denials |= DIR_EXEC;
                if (mode & S_IWOTH) denials |= DIR_WRITE;
                if (mode & S_IROTH) denials |= DIR_READ;
            }
            else
            {
                pdace->flags = FILE_INHERITANCE;
                if (mode & S_IXOTH) denials |= FILE_EXEC;
                if (mode & S_IWOTH) denials |= FILE_WRITE;
                if (mode & S_IROTH) denials |= FILE_READ;
            }
            denials &= ~(grants | OWNER_RIGHTS);
            if (denials)
            {
    ...
    if (isdir)
    {
        ...
        if (mode & S_IXOTH) grants |= DIR_EXEC;
        if (mode & S_IWOTH) grants |= DIR_WRITE;
        if (mode & S_IROTH) grants |= DIR_READ;
    }
    else
    {
        pgace->flags = FILE_INHERITANCE;
        if (mode & S_IXOTH) grants |= FILE_EXEC;
        if (mode & S_IWOTH) grants |= FILE_WRITE;
        if (mode & S_IROTH) grants |= FILE_READ;
    }
    pgace->size = cpu_to_le16(wsidsz + 8);
    pgace->mask = grants;
    memcpy((char*) &pgace->sid, worldsid, wsidsz);
    ...
    if (isdir)
        pgace->flags = DIR_INHERITANCE;
    else pgace->flags = FILE_INHERITANCE;
    pgace->size = cpu_to_le16(asidsz + 8);
    grants = OWNER_RIGHTS | FILE_READ | FILE_WRITE | FILE_EXEC;
    pgace->mask = grants;
    memcpy((char*) &pgace->sid, adminsid, asidsz);
    ...
    if (mode & (S_ISVTX | S_ISGID | S_ISUID))
    {
        ...
        pgace->size = cpu_to_le16(nsidsz + 8);
        grants = const_cpu_to_le32(0);
        if (mode & S_ISUID) grants |= FILE_APPEND_DATA;
        if (mode & S_ISGID) grants |= FILE_WRITE_DATA;
        if (mode & S_ISVTX) grants |= FILE_READ_DATA;
        pgace->mask = grants;
        memcpy((char*) &pgace->sid, nullsid, nsidsz);
        pos += nsidsz + 8;
        acecnt++;
...
char *ntfs_build_descr(mode_t mode, int isdir, const SID * usid, const SID * gsid)
{
    ...
    if (newattr)
    {
         ...
         * to be changed in an inheritance after creation
         */
        pnhead->control = SE_DACL_PRESENT | SE_DACL_PROTECTED | SE_SELF_RELATIVE;
        /*
         * Windows prefers ACL first, do the same to
         * get the same hash value and avoid duplication
         */
        /* build permissions */
        aclsz = buildacls(newattr, sizeof(SECURITY_DESCRIPTOR_RELATIVE), mode, isdir, usid, gsid);
        if (((int) sizeof(SECURITY_DESCRIPTOR_RELATIVE) + aclsz + usidsz + gsidsz) <= newattrsz)
        {
            /* append usid and gsid */
            memcpy(&newattr[sizeof(SECURITY_DESCRIPTOR_RELATIVE) + aclsz], usid, usidsz);
            memcpy(&newattr[sizeof(SECURITY_DESCRIPTOR_RELATIVE) + aclsz + usidsz], gsid, gsidsz);
            /* positions of ACL, USID and GSID into header */
            pnhead->owner = cpu_to_le32(sizeof(SECURITY_DESCRIPTOR_RELATIVE) + aclsz);
            pnhead->group = cpu_to_le32(sizeof(SECURITY_DESCRIPTOR_RELATIVE) + aclsz + usidsz);
            pnhead->sacl = const_cpu_to_le32(0);
            pnhead->dacl = const_cpu_to_le32(sizeof(SECURITY_DESCRIPTOR_RELATIVE));
        }
        else
        {
            /* hope error was detected before overflowing */
...
static int merge_permissions(BOOL isdir, le32 owner, le32 group, le32 world, le32 special)

{
    ...
    if (owner)
    {
        if (isdir)
        {
            ...
            if (owner & DIR_GEXEC) perm |= S_IXUSR;
            /* write if any of addfile, adddir, delchild */
            if (owner & DIR_GWRITE) perm |= S_IWUSR;
            /* read if any of list */
            if (owner & DIR_GREAD) perm |= S_IRUSR;
        }
        else
        {
            /* exec if execute or generic execute */
            if (owner & FILE_GEXEC) perm |= S_IXUSR;
            /* write if any of writedata or generic write */
            if (owner & FILE_GWRITE) perm |= S_IWUSR;
            /* read if any of readdata or generic read */
            if (owner & FILE_GREAD) perm |= S_IRUSR;
        }
    }
    ...
    if (group)
    {
        ...
        {
            /* exec if any of list, traverse */
            if (group & DIR_GEXEC) perm |= S_IXGRP;
            /* write if any of addfile, adddir, delchild */
            if (group & DIR_GWRITE) perm |= S_IWGRP;
            /* read if any of list */
            if (group & DIR_GREAD) perm |= S_IRGRP;
        }
        else
        {
            /* exec if execute */
            if (group & FILE_GEXEC) perm |= S_IXGRP;
            /* write if any of writedata, appenddata */
            if (group & FILE_GWRITE) perm |= S_IWGRP;
            /* read if any of readdata */
            if (group & FILE_GREAD) perm |= S_IRGRP;
        }
    }
    ...
    if (world)
    {
        ...
        {
            /* exec if any of list, traverse */
            if (world & DIR_GEXEC) perm |= S_IXOTH;
            /* write if any of addfile, adddir, delchild */
            if (world & DIR_GWRITE) perm |= S_IWOTH;
            /* read if any of list */
            if (world & DIR_GREAD) perm |= S_IROTH;
        }
        else
        {
            /* exec if execute */
            if (world & FILE_GEXEC) perm |= S_IXOTH;
            /* write if any of writedata, appenddata */
            if (world & FILE_GWRITE) perm |= S_IWOTH;
            /* read if any of readdata */
            if (world & FILE_GREAD) perm |= S_IROTH;
        }
    }
    /* build special permission flags */
    if (special)
    {
        if (special & FILE_APPEND_DATA) perm |= S_ISUID;
        if (special & FILE_WRITE_DATA) perm |= S_ISGID;
        if (special & FILE_READ_DATA) perm |= S_ISVTX;
    }
    return (perm);
}
...
static int build_std_permissions(const char *securattr, const SID *usid, const SID *gsid, BOOL isdir)
{
    const SECURITY_DESCRIPTOR_RELATIVE *phead;
    const ACL *pacl;
    const ACCESS_ALLOWED_ACE *pace;
    ...
    for (nace = 0; nace < acecnt; nace++)
    {
        ...
        {
            if (ntfs_same_sid(usid, &pace->sid) || ntfs_same_sid(ownersid, &pace->sid))
            {
                noown = FALSE;
                if (pace->type == ACCESS_ALLOWED_ACE_TYPE)
                    allowown |= pace->mask;
                else if (pace->type == ACCESS_DENIED_ACE_TYPE) denyown |= pace->mask;
            }
            else if (ntfs_same_sid(gsid, &pace->sid) && !(pace->mask & WRITE_OWNER))
            {
                if (pace->type == ACCESS_ALLOWED_ACE_TYPE)
                    allowgrp |= pace->mask;
                else if (pace->type == ACCESS_DENIED_ACE_TYPE) denygrp |= pace->mask;
            }
            else if (is_world_sid((const SID*) &pace->sid))
            {
                if (pace->type == ACCESS_ALLOWED_ACE_TYPE)
                    allowall |= pace->mask;
                else if (pace->type == ACCESS_DENIED_ACE_TYPE) denyall |= pace->mask;
            }
            else if ((ntfs_same_sid((const SID*) &pace->sid, nullsid)) && (pace->type == ACCESS_ALLOWED_ACE_TYPE)) special
                    |= pace->mask;
        }
     ...
     */
    if (noown) allowown = (FILE_READ_DATA | FILE_WRITE_DATA | FILE_EXECUTE);
    /*
    ...
    allowgrp |= allowall;
    return (merge_permissions(isdir, allowown & ~(denyown | denyall), allowgrp & ~(denygrp | denyall), allowall
            & ~denyall, special));
}
...
static int build_owngrp_permissions(const char *securattr, const SID *usid, BOOL isdir)
{
    const SECURITY_DESCRIPTOR_RELATIVE *phead;
    const ACL *pacl;
    const ACCESS_ALLOWED_ACE *pace;
    ...
    for (nace = 0; nace < acecnt; nace++)
    {
        ...
        if (!(pace->flags & INHERIT_ONLY_ACE))
        {
            if ((ntfs_same_sid(usid, &pace->sid) || ntfs_same_sid(ownersid, &pace->sid)) && (pace->mask & WRITE_OWNER))
            {
                if (pace->type == ACCESS_ALLOWED_ACE_TYPE) allowown |= pace->mask;
            }
            else if (ntfs_same_sid(usid, &pace->sid) && (!(pace->mask & WRITE_OWNER)))
            {
                if (pace->type == ACCESS_ALLOWED_ACE_TYPE)
                {
                    allowgrp |= pace->mask;
    ...
    if (!grppresent) allowgrp = allowall;
    return (merge_permissions(isdir, allowown & ~(denyown | denyall), allowgrp & ~(denygrp | denyall), allowall
            & ~denyall, special));
}
...
static int build_ownadmin_permissions(const char *securattr, const SID *usid, const SID *gsid, BOOL isdir)
{
    const SECURITY_DESCRIPTOR_RELATIVE *phead;
    const ACL *pacl;
    const ACCESS_ALLOWED_ACE *pace;
    ...
    {
        pace = (const ACCESS_ALLOWED_ACE*) &securattr[offace];
        if (!(pace->flags & INHERIT_ONLY_ACE) && !(~pace->mask & (ROOT_OWNER_UNMARK | ROOT_GROUP_UNMARK)))
        {
            if ((ntfs_same_sid(usid, &pace->sid) || ntfs_same_sid(ownersid, &pace->sid))
        ...
        else if (!(pace->flags & INHERIT_ONLY_ACE)) if ((ntfs_same_sid((const SID*) &pace->sid, nullsid))
                && (pace->type == ACCESS_ALLOWED_ACE_TYPE)) special |= pace->mask;
        offace += le16_to_cpu(pace->size);
    }
    if (isforeign)
    {
...
static uid_t find_tenant(struct MAPPING * const mapping[], const char *securattr)
{
    const SECURITY_DESCRIPTOR_RELATIVE *phead;
    const ACL *pacl;
    const ACCESS_ALLOWED_ACE *pace;
    ...
    {
        pace = (const ACCESS_ALLOWED_ACE*) &securattr[offace];
        if ((pace->type == ACCESS_ALLOWED_ACE_TYPE) && (pace->mask & DIR_WRITE))
        {
...
int ntfs_build_permissions(const char *securattr, const SID *usid, const SID *gsid, BOOL isdir)
{
    ...
    else perm = build_std_permissions(securattr, usid, gsid, isdir);
    return (perm);
}
...
static SID *encodesid(const char *sidstr)
{
    ...
    if (!strncmp(sidstr, "S-1-", 4))
    {
        ...
        if ((cnt > 0) && ntfs_valid_sid(bsid) && ntfs_is_user_sid(bsid))
        {
            ...
            if (sid) memcpy(sid, bsid, 4 * cnt + 8);
        }
    }
    return (sid);
}
...
static struct MAPLIST *getmappingitem(FILEREADER reader, void *fileid, off_t *poffs, char *buf, int *psrc, s64 *psize)
{
    int src;
    int dst;
    char *p;
    ...
    if (item)
    {
        ...
        {
            gotend = 0;
            while ((src < *psize) && (buf[src] != '\n'))
            {
                if (dst < LINESZ) item->maptext[dst++] = buf[src];
                src++;
            }
            if (src >= *psize)
            {
...
struct MAPLIST *ntfs_read_mapping(FILEREADER reader, void *fileid)
{
    ...
    if (size > 0)
    {
        ...
        do
        {
            item = getmappingitem(reader, fileid, &offs, buf, &src, &size);
            if (item)
            {
...
void ntfs_free_mapping(struct MAPPING *mapping[])
{
    ...
    while (mapping[MAPUSERS])
    {
        ...
        while (group && (group->sid != user->sid))
            group = group->next;
        if (!group) free(user->sid);
        /* free group list if any */
        if (user->grcnt) free(user->groups);
    ...
    for (item = firstitem; item; item = item->next)
    {
        ...
        else
        {
            ...
            if (item->uidstr[0])
            {
                ...
                if (pwd)
                    uid = pwd->pw_uid;
                else log_early_error("Invalid user \"%s\"\n", item->uidstr);
            }
        }
        /*
         ...
         */
        if (uid || (!item->uidstr[0] && !item->gidstr[0]))
        {
            sid = encodesid(item->sidstr);
            if (sid && !item->uidstr[0] && !item->gidstr[0] && !ntfs_valid_pattern(sid))
            {
            ...
            if (sid)
            {
                mapping = (struct MAPPING*) ntfs_malloc(sizeof(struct MAPPING));
                if (mapping)
                {
                    mapping->sid = sid;
                    mapping->xid = uid;
...
struct MAPPING *ntfs_do_group_mapping(struct MAPLIST *firstitem)
{
    ...
    for (step = 1; step <= 2; step++)
    {
        for (item = firstitem; item; item = item->next)
        {
            secondstep = (item->uidstr[0] != '\0') || !item->gidstr[0];
            ok = (step == 1 ? !secondstep : secondstep);
            if ((item->gidstr[0] >= '0') && (item->gidstr[0] <= '9'))
            ...
            else
            {
                ...
                if (item->gidstr[0])
                {
                    ...
                    if (grp)
                        gid = grp->gr_gid;
                    else log_early_error("Invalid group \"%s\"\n", item->gidstr);
                }
            }
            /*
             ...
             */
            if (ok && (gid || (!item->uidstr[0] && !item->gidstr[0])))
            {
                sid = encodesid(item->sidstr);
                if (sid && !item->uidstr[0] && !item->gidstr[0] && !ntfs_valid_pattern(sid))
                {
