...
static int __ntfs_volume_release(ntfs_volume *v)
{
    ...
    if (ntfs_inode_free(&v->vol_ni)) ntfs_error_set(&err);
    /*
     * FIXME: Inodes must be synced before closing
     * attributes, otherwise unmount could fail.
     */
    if (v->lcnbmp_ni && NInoDirty(v->lcnbmp_ni)) ntfs_inode_sync(v->lcnbmp_ni);
    ntfs_attr_free(&v->lcnbmp_na);
    if (ntfs_inode_free(&v->lcnbmp_ni)) ntfs_error_set(&err);

    if (v->mft_ni && NInoDirty(v->mft_ni)) ntfs_inode_sync(v->mft_ni);
    ntfs_attr_free(&v->mftbmp_na);
    ntfs_attr_free(&v->mft_na);
    if (ntfs_inode_free(&v->mft_ni)) ntfs_error_set(&err);

    if (v->mftmirr_ni && NInoDirty(v->mftmirr_ni)) ntfs_inode_sync(v->mftmirr_ni);
    ntfs_attr_free(&v->mftmirr_na);
    if (ntfs_inode_free(&v->mftmirr_ni)) ntfs_error_set(&err);

    if (v->dev)
    {
        struct ntfs_device *dev = v->dev;

        if (dev->d_ops->sync(dev)) ntfs_error_set(&err);
        if (dev->d_ops->close(dev)) ntfs_error_set(&err);
    }
...
static int ntfs_mft_load(ntfs_volume *vol)
{
    ...
    if (ntfs_mft_record_check(vol, 0, mb)) goto error_exit;

    ctx = ntfs_attr_get_search_ctx(vol->mft_ni, NULL);
    if (!ctx) goto error_exit;
    ...
    if (!vol->mft_ni->attr_list) goto error_exit;
...
ntfs_volume *ntfs_volume_startup(struct ntfs_device *dev, unsigned long flags)
{
    ...
    if (mft_lcn * vol->cluster_size < 16 * 1024) mft_lcn = (16 * 1024 + vol->cluster_size - 1) / vol->cluster_size;
    ...
    error_exit: eo = errno;
    free(bs);
    if (vol) __ntfs_volume_release(vol);
    errno = eo;
    return NULL;
}
...
static int ntfs_volume_check_logfile(ntfs_volume *vol)
{
    ...
    if (!ntfs_check_logfile(na, &rp) || !ntfs_is_logfile_clean(na, rp)) err = EOPNOTSUPP;
    free(rp);
    ntfs_attr_close(na);
    out: if (ntfs_inode_close(ni)) ntfs_error_set(&err);
    if (err)
    {
        errno = err;
        return -1;
...
static ntfs_inode *ntfs_hiberfile_open(ntfs_volume *vol)
{
    ...
    out: if (ntfs_inode_close(ni_root))
    {
...
int ntfs_volume_check_hiberfile(ntfs_volume *vol, int verbose)
{
    ...
    {
        if (verbose) ntfs_log_error("Hibernated non-system partition, "
                "refused to mount.\n");
        errno = EPERM;
        goto out;
    }
    ...
    errno = 0;
    out: if (na) ntfs_attr_close(na);
    free(buf);
    err = errno;
    if (ntfs_inode_close(ni)) ntfs_error_set(&err);
    errno = err;
    return errno ? -1 : 0;
}
    ...
    else
    {
        ...
        if (na)
        {
            if (NAttrNonResident(na))
            {
                ...
                txf_data = ntfs_attr_readall(ni, AT_LOGGED_UTILITY_STREAM, TXF_DATA, 9, &txf_data_size);
                if (txf_data)
                {
                    if (ntfs_attr_truncate(na, 0)
                            || (ntfs_attr_pwrite(na, 0, txf_data_size, txf_data) != txf_data_size)) res = -1;
                    free(txf_data);
                }
                if (res)
...
ntfs_volume *ntfs_device_mount(struct ntfs_device *dev, unsigned long flags)
{
    ...
    for (i = 0; i < vol->mftmirr_size; ++i)
    {
        ...
        const char *ESTR[12] = { "$MFT", "$MFTMirr", "$LogFile", "$Volume", "$AttrDef", "root directory", "$Bitmap",
    ...
    /* Sanity checks. */
    if ((char*) vinf + le32_to_cpu(a->value_length) > (char*) ctx->mrec + le32_to_cpu(ctx->mrec->bytes_in_use)
    ...
    if (ntfs_attr_lookup(AT_VOLUME_NAME, AT_UNNAMED, 0, 0, 0, NULL, 0, ctx))
    {
        ...
        if (!vol->vol_name) goto error_exit;
        vol->vol_name[0] = '\0';
    }
    else
    {
    ...
    if (!vol->attrdef) goto error_exit;
    /* Read in the $DATA attribute value into the buffer. */
    ...
    if (!(flags & MS_RDONLY))
    {
        if (!(flags & MS_IGNORE_HIBERFILE) && ntfs_volume_check_hiberfile(vol, 1) < 0) goto error_exit;
        if (ntfs_volume_check_logfile(vol) < 0)
        {
            if (!(flags & MS_RECOVER)) goto error_exit;
            ntfs_log_info("The file system wasn't safely "
                    "closed on Windows. Fixing.\n");
            if (ntfs_logfile_reset(vol)) goto error_exit;
        }
    }
    /* make $TXF_DATA resident if present on the root directory */
    if (!NVolReadOnly(vol) && fix_txf_data(vol)) goto error_exit;
...
int ntfs_set_shown_files(ntfs_volume *vol, BOOL show_sys_files, BOOL show_hid_files, BOOL hide_dot_files)
{
    ...
    if (vol)
    {
        ...
        NVolClearHideDotFiles(vol);
        if (show_sys_files) NVolSetShowSysFiles(vol);
        if (show_hid_files) NVolSetShowHidFiles(vol);
        if (hide_dot_files) NVolSetHideDotFiles(vol);
        res = 0;
    }
    if (res) ntfs_log_error("Failed to set file visibility\n");
    return (res);
}
...
int ntfs_set_ignore_case(ntfs_volume *vol)
{
    ...
    if (vol && vol->upcase)
    {
        vol->locase = ntfs_locase_table_build(vol->upcase, vol->upcase_len);
        if (vol->locase)
        {
    ...
    if (res) ntfs_log_error("Failed to set ignore_case mode\n");
    return (res);
}
 ...
 */
ntfs_volume *ntfs_mount(const char *name __attribute__((unused)), unsigned long flags __attribute__((unused)))
{
    ...
    else ntfs_create_lru_caches(vol);
    return vol;
#else
    /*
...
int ntfs_logfile_reset(ntfs_volume *vol)
{
    ...
    error_exit: ntfs_inode_close(ni);
    errno = eo;
    return -1;
}
...
int ntfs_volume_write_flags(ntfs_volume *vol, const le16 flags)
{
    ...
    /* Sanity checks. */
    if ((char*) c + le32_to_cpu(a->value_length) > (char*) ctx->mrec + le32_to_cpu(ctx->mrec->bytes_in_use)
    ...
    if (ntfs_inode_sync(vol->vol_ni)) goto err_out;
...
int ntfs_volume_get_free_space(ntfs_volume *vol)
{
    ...
    else
    {
        ...
        if (vol->free_mft_records >= 0) vol->free_mft_records += (na->allocated_size - na->data_size) << 3;
